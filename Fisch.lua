...
local function SafeNotify(tbl)
    if Library and Library.MakeNotify then
        Library:MakeNotify(tbl)
    else
        print("[ZuperMing] " .. (tbl.Title or "") .. ": " .. (tbl.Content or ""))
    end
end
    ...
    if ReelModule then
        ...
        if success and Controller then
            ...
            Controller.EndMinigame = function(self, result)
                ...
                if not ok then
                    warn("[ZuperMing] EndMinigame pcall error (harmless): " .. tostring(res))
                end
                return res
            end
...
local SpearNet = ReplicatedStorage:WaitForChild("packages"):WaitForChild("Net")
local SpearEvent = SpearNet:WaitForChild("RE/SpearFishing/Minigame", 10)
if not SpearEvent then
    for _, v in pairs(SpearNet:GetDescendants()) do
        if v.Name == "Minigame" and v.Parent and v.Parent.Name == "SpearFishing" then
            SpearEvent = v
            break
        end
    end
...
local function RunAutoSpear()
    if not SpearEvent then
        SafeNotify({Title = "Auto Spear", Content = "SpearFishing Remote tidak ditemukan!", Duration = 4})
        Config.AutoSpear = false
        return
    end
...
local function equipAndUseTotem(name)
    ...
    for _, tool in pairs(bp:GetChildren()) do
        if tool:IsA("Tool") and tool.Name == name then
            ...
            SafeNotify({Title = "Auto Totem", Content = "Totem activated!", Duration = 3})
            task.wait(1)
    ...
    SafeNotify({Title = "Auto Totem", Content = "Totem tidak ditemukan!", Duration = 4})
    Config.TotemActive = false
end
...
local TotemBusyTimer = 0
local TOTEM_BUSY_DURATION = 5
...
local function ParseAppraiseResult(text)
    ...
    local hasBig = cleanText:find("Big") ~= nil
...
local function RunAutoAppraise()
    ...
    task.spawn(function()
        ...
        if prompt then
            ...
            SafeNotify({Title = "Error", Content = "Gagal interaksi dengan NPC!", Duration = 3})
            Config.AutoAppraise = false
            return
        end
...
local function FetchInventory()
    local ok, result = pcall(function()
        return DataController.PlayerDataReplicator:Index({"Inventory"})
    end)
    if ok and result then return result end
    local ok2, result2 = pcall(function()
        return DataController.fetch("Inventory")
    end)
...
local function RefreshDropdownSilent()
    ...
    pcall(function() if DropStorage then DropStorage:Refresh(MakeStorageNameOptions()) end end)
end
...
local function RunAutoStorage()
    ...
    if not lastCFrame then
        SafeNotify({Title = "Error", Content = "Character tidak ditemukan!", Duration = 3})
        return
    end
...
task.spawn(function()
    ...
    pcall(function()
        if DropStorage then
            DropStorage:Refresh(MakeStorageNameOptions())
        end
    end)
end)
...
Library.MakeNotify = function(_, tbl)
    Library:SetNotification({
        Title = tbl.Title or "ZuperMing",
        Description = "| Info",
        Content = tbl.Content or "",
        Time = 0.5,
        Delay = tbl.Duration or 3,
    })
end

local Window = Library:CreateWindow({
    Title = "ZuperMing",
    Description = "| Fisch | v1.3",
    ["Tab Width"] = 120,
    Acrylic = false,
    Theme = "Grey"
})

local DiscordTab = Window:CreateTab({ Name = "Discord", Icon = "discord" })
local InfoSection = DiscordTab:AddSection("Infomation", true)
...
local MainTab = Window:CreateTab({ Name = "Main", Icon = "landmark" })
...
FarmSection:AddDropdown({
    ...
    Multi = false,
    Callback = function(v)
        Config.Mode = type(v) == "table" and v[1] or v
    end,
})
...
FarmSection:AddToggle({
    ...
    Default = false,
    Callback = function(v) Config.InstantBobber = v end,
})
...
local SnapSection = MainTab:AddSection("Snap Reel", false)
...
SnapSection:AddDropdown({
    ...
    Multi = false,
    Callback = function(v)
        Config.SnapFishName = type(v) == "table" and v[1] or v
    end,
})
...
SnapSection:AddDropdown({
    ...
    Multi = false,
    Callback = function(v)
        Config.SnapShinyType = type(v) == "table" and v[1] or v
    end,
})
...
SnapSection:AddDropdown({
    ...
    Multi = false,
    Callback = function(v)
        Config.SnapMutation = type(v) == "table" and v[1] or v
    end,
})
...
local SpearSection = MainTab:AddSection("Auto Spear Fishing", false)
...
SpearSection:AddDropdown({
    ...
    Multi = false,
    Callback = function(v)
        Config.AutoSpearLocation = type(v) == "table" and v[1] or v
        if Config.AutoSpear then
            ...
            if char and char:FindFirstChild("HumanoidRootPart") and targetCFrame then
                ...
                Library:MakeNotify({
                    Title = "Spear Location",
                    Content = "Pindah ke: " .. Config.AutoSpearLocation,
                    Duration = 2
                })
            end
        end
...
SpearSection:AddToggle({
    ...
    Callback = function(v)
        ...
        if v then
            ...
            Library:MakeNotify({Title = "Auto Spear", Content = "Stopped.", Duration = 2})
        end
    end,
})

local AutoTab = Window:CreateTab({ Name = "Automatically", Icon = "play" })
local SellSection = AutoTab:AddSection("Selling Config", false)

SellSection:AddInput({
    Title = "Sell Interval (s)",
    Default = "5",
    Callback = function(v) Config.SellInterval = tonumber(v) or 5 end,
})
...
local AppraiseSection = AutoTab:AddSection("Auto Appraise", true)

AppraiseSection:AddDropdown({
    Title = "Tier Requirement",
    Options = {"None", "Shiny", "Sparkling", "ShinySparkling"},
    Default = {"None"},
    Multi = false,
    Callback = function(v)
        Config.ShinyType = type(v) == "table" and v[1] or v
    end,
})
...
AppraiseSection:AddDropdown({
    ...
    Multi = false,
    Callback = function(v)
        Config.TargetSize = type(v) == "table" and v[1] or v
    end,
})
...
AppraiseSection:AddDropdown({
    ...
    Multi = false,
    Callback = function(v)
        Config.TargetMutation = type(v) == "table" and v[1] or v
    end,
})

AppraiseSection:AddToggle({
...
local TotemSection = AutoTab:AddSection("Auto Totem", true)
...
TotemSection:AddDropdown({
    ...
    Multi = false,
    Callback = function(v)
        Config.TotemDay = type(v) == "table" and v[1] or v
    end,
})
...
TotemSection:AddDropdown({
    ...
    Multi = false,
    Callback = function(v)
        Config.TotemNight = type(v) == "table" and v[1] or v
    end,
})
...
TotemSection:AddInput({
    ...
    Callback = function(v)
        ...
        if val and val > 0 then Config.TotemDelay = val end
    end,
})
...
TotemSection:AddToggle({
    ...
    Callback = function(v)
        ...
        if v then
            RunAutoTotem()
            SafeNotify({Title = "Auto Totem", Content = "Smart Totem Started!", Duration = 3})
        else
            if TotemThread then task.cancel(TotemThread) end
            Config.TotemBusy = false
            Config.TotemActive = false
            SafeNotify({Title = "Auto Totem", Content = "Stopped.", Duration = 3})
        end
    end,
})

local EnchantSection = AutoTab:AddSection("Auto Enchant System", true)
...
local function UpdateEnchantDropdown(relicType)
    ...
    pcall(function() EnchantDropdownRef:Refresh(list) end)
end
...
EnchantSection:AddDropdown({
    ...
    Multi = false,
    Callback = function(v)
        Config.SelectedRelic = type(v) == "table" and v[1] or v
        UpdateEnchantDropdown(Config.SelectedRelic)
    end,
})
...
EnchantDropdownRef = EnchantSection:AddDropdown({
    ...
    Multi = false,
    Callback = function(v)
        Config.TargetEnchant = type(v) == "table" and v[1] or v
    end,
})
...
local FavSection = AutoTab:AddSection("Auto Favourite", true)
...
FavSection:AddButton({
    ...
    Callback = function()
        BuildFishNames()
        SelName = {}
        pcall(function() DropName:Refresh(MakeNameOptions()) end)
        local total = 0
        for _, uids in pairs(FishByName) do total += #uids end
        Library:MakeNotify({Title = "Refresh", Content = total .. " ikan ditemukan!", Duration = 3})
    end,
})
...
FavSection:AddToggle({
    ...
    Callback = function(v)
        ...
        if not hasFilter then
            Library:MakeNotify({Title = "Warning", Content = "Pilih minimal 1 filter dulu!", Duration = 3})
            return
        end
        local uids = CollectUIDs()
        local count = 0
        for _, uid in pairs(uids) do
            pcall(function() Backpack.handleInput(uid, "favourite", true) end)
            count += 1
            task.wait(0.05)
        end
        Library:MakeNotify({Title = "Auto Favourite", Content = count .. " ikan berhasil di-favourite!", Duration = 4, Icon = "star"})
    end,
})

local StorageSection = AutoTab:AddSection("Auto Fish Storage", true)
...
DropStorage = StorageSection:AddDropdown({
    ...
    Callback = function(Value)
        ...
        for k, v in pairs(Value) do
            local entry = type(k) == "number" and v or (v == true and k or nil)
            if entry and entry ~= "None" then table.insert(t, entry) end
        end
        StorageSelName = t
    end,
})
    ...
    Callback = function(v)
        if v then
            if #StorageSelName == 0 then
                Library:MakeNotify({Title = "Warning", Content = "Pilih minimal 1 ikan dulu!", Duration = 3})
                return
            end
            StorageActive = true
            task.spawn(RunAutoStorage)
            Library:MakeNotify({Title = "Auto Storage", Content = "Memulai proses storage...", Duration = 2})
        else
            StorageActive = false
        end
    end,
...
local ChestSection = AutoTab:AddSection("Auto Collect All Chest", true)
...
ChestSection:AddButton({
    ...
    Callback = function() AutoCollectAllChests() end,
})

local StarSection = AutoTab:AddSection("Auto Collect All Starfall", true)

StarSection:AddButton({
    Title = "Claim Starfall",
    Content = "Claim semua item di Starfall lalu kembali ke posisi asal",
    Callback = function() task.spawn(RunStarCrater) end,
})

local ShopTab = Window:CreateTab({ Name = "Shop", Icon = "shopping-bag" })
local BuyRodSection = ShopTab:AddSection("Buy Rods", true)
...
BuyRodSection:AddDropdown({
    ...
    Multi = false,
    Callback = function(v)
        Config.SelectedRod = type(v) == "table" and v[1] or v
    end,
})
...
BuyRodSection:AddButton({
    ...
    Callback = function()
        if Config.SelectedRod then
            ReplicatedStorage.events.purchase:FireServer(Config.SelectedRod, "Rod", nil, 1)
            Library:MakeNotify({Title = "Shop", Content = "Buying " .. Config.SelectedRod .. "...", Duration = 3, Icon = "cart"})
        end
    end,
})

local TotemShopSection = ShopTab:AddSection("Auto Buy Totem", true)
...
TotemShopSection:AddDropdown({
    ...
    Multi = false,
    Callback = function(v)
        Config.SelectedTotem = type(v) == "table" and v[1] or v
        local price = TotemData[Config.SelectedTotem] or "Unknown"
        Library:MakeNotify({
            Title = "Totem Selected",
            Content = Config.SelectedTotem .. " harganya " .. price,
            Duration = 3,
            Icon = "shopping-cart"
        })
    end,
...
TotemShopSection:AddToggle({
    Title = "Enable Auto Buy",
    Default = false,
    Callback = function(v) Config.AutoBuyTotem = v end,
})

local BaitSection = ShopTab:AddSection("Auto Buy Crate", true)
...
BaitSection:AddDropdown({
    ...
    Multi = false,
    Callback = function(v)
        Config.SelectedShopItem = type(v) == "table" and v[1] or v
    end,
})
...
BaitSection:AddInput({
    Title = "Amount to Buy",
    Default = "1",
    Callback = function(v) Config.BuyItemAmount = tonumber(v) or 1 end,
})
...
BaitSection:AddButton({
    ...
    Callback = function()
        if Config.SelectedShopItem then
            ...
            for i = 1, amount do
                game:GetService("ReplicatedStorage").events.purchase:FireServer(Config.SelectedShopItem, "Fish", nil, 1)
...
local RemoteShopSection = ShopTab:AddSection("Item Shop", true)
...
local selectedPurchaseItem = "Basic Diving Gear"
...
RemoteShopSection:AddDropdown({
    ...
    Multi = false,
    Callback = function(v)
        selectedPurchaseItem = type(v) == "table" and v[1] or v
    end,
})
...
RemoteShopSection:AddButton({
    ...
    Callback = function()
        ...
        if purchaseEvent then
            ...
            Library:MakeNotify({Title = "Purchase", Content = "Attempting to buy: " .. selectedPurchaseItem, Duration = 3})
        else
...
local ShopUISection = ShopTab:AddSection("Shop UI", true)
...
local TeleportSection = TeleportTab:AddSection("Locations", true)
...
TeleportSection:AddDropdown({
    ...
    Multi = false,
    Callback = function(v)
        Config.SelectedLoc = Locations[type(v) == "table" and v[1] or v]
    end,
})
...
local WhirlpoolSection = TeleportTab:AddSection("Teleport Whirlpool", true)
            ...
            if targetWhirlpool then
                ...
                if hrp then
                    ...
                    if targetWhirlpool:IsA("Model") then
                        targetCFrame = targetWhirlpool.PrimaryPart and targetWhirlpool.PrimaryPart.CFrame or targetWhirlpool:GetModelCFrame()
                    elseif targetWhirlpool:IsA("BasePart") then
                        targetCFrame = targetWhirlpool.CFrame
                    end
                    if targetCFrame then
                        hrp.CFrame = targetCFrame * CFrame.new(0, 5, 0)
                        Library:MakeNotify({Title = "Success", Content = "Teleported to Safe Whirlpool!", Duration = 3})
                    end
                end
            else
                Library:MakeNotify({Title = "Whirlpool Not Found", Content = "Whirlpool sedang tidak aktif di server ini.", Duration = 3})
            end
        end
    end,
})

local TeleportPlayer = TeleportTab:AddSection("Teleport Player", true)
...
local function GetTargetCFrame(targetName)
    ...
    if pivot and pivot.Position.Y > -500 then return pivot end
    local hrp = char:FindFirstChild("HumanoidRootPart")
...
local function updatePlayerList()
    ...
    if #playerList == 0 then table.insert(playerList, "No other players") end

    if _G.TelePlayer then
        pcall(function() _G.TelePlayer:Refresh(playerList) end)
...
_G.TelePlayer = TeleportPlayer:AddDropdown({
    ...
    Multi = false,
    Callback = function(v)
        selectedPlayerName = type(v) == "table" and v[1] or v
    end,
})
updatePlayerList()
...
local MiscTab = Window:CreateTab({ Name = "Misc", Icon = "misc" })
local IdentitySection = MiscTab:AddSection("Hide Identity", true)
...
local PlayerUtilities = MiscTab:AddSection("Player Utilities", true)

PlayerUtilities:AddToggle({
    Title = "Enable Radar Bypass",
    Default = false,
    Callback = function(v) ToggleRadar(v) end,
})
...
local function startWalkOnWater()
    ...
    wowLoop = RunService.Heartbeat:Connect(function()
        ...
        if root and humanoid then
            ...
            local rayResult = Workspace:Raycast(rayOrigin, Vector3.new(0, -500, 0), raycastParams)
...
local GeneralFeatures = MiscTab:AddSection("General Features", true)
...
GeneralFeatures:AddToggle({
    Title = "Anti AFK",
    Content = "Prevent kick 20 minutes",
    Default = true,
    Callback = function(v)
        if v then
            StartAntiAFKLogic()
...
SettingsSection:AddDropdown({
    ...
    Callback = function(v)
        ...
        if selected and selected ~= "" then Library:SetTheme(selected) end
    end,
})
