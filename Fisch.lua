...
local MainTab = Window:CreateTab({ Name = "Main", Icon = "fish" })
local FarmSection = MainTab:AddSection("Auto Farm", true)
...
local SnapSection = MainTab:AddSection("Snap Reel", true)
...
local SpearSection = MainTab:AddSection("Auto Spear Fishing", true)
...
local AutoTab = Window:CreateTab({ Name = "Auto", Icon = "zap" })
local SellSection = AutoTab:AddSection("Selling Config", true)
...
FavSection:AddButton({
    ...
    Callback = function()
        BuildFishNames()
        SelName = {}
        pcall(function() DropName:SetValues(MakeNameOptions()) end)
        local total = 0
...
local function updatePlayerList()
    ...
    if _G.TelePlayer then
        _G.TelePlayer:Clear()
        for i = 1, #playerList do _G.TelePlayer:AddOption(playerList[i]) end
