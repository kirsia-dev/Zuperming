...
local Zuper_CrosshairGui, Zuper_CrossH, Zuper_CrossV = nil, nil, nil
...
local function GetSafeChamsFolder()
    ...
    local f = pg:FindFirstChild("Zuper_WorkspaceChams")
    if not f then
        f = Instance.new("Folder")
        f.Name = "Zuper_WorkspaceChams"
        f.Parent = pg
    end
    VD_ChamsFolder = f
    return f
...
local ConfigFolderName = "ZuperMing_VD_Configs"
...
local function Zuper_SaveConfig(name)
...
local function Zuper_LoadConfig(name)
    name = (name and name ~= "") and name or getgenv().CurrentConfigName
    if not name or name == "" then name = "Default" end
    local path = ConfigFolderName .. "/" .. name .. ".json"
    pcall(function()
...
local function Zuper_DeleteConfig(name)
    name = (name and name ~= "") and name or getgenv().CurrentConfigName
...
pcall(function() Zuper_LoadConfig("Default") end)
...
local function setupAntiFail()
    ...
    task.spawn(function()
        local ok, err = pcall(function()
            ...
            local GenStart   = GenFolder and GenFolder:FindFirstChild("RepairEvent")
            
            local HealFolder = Events and Events:FindFirstChild("Healing")
            if not HealFolder then HealFolder = Remotes and Remotes:FindFirstChild("Healing") end
            ...
            local HealStart  = HealFolder and HealFolder:FindFirstChild("HealEvent")
            ...
            oldNamecall      = hookmetamethod(game, "__namecall", function(self, ...)
                ...
                if GenStart and VD.InstantGen and self == GenStart and method == "FireServer" then
                    local targetPoint = args[1]
                    local isStarting = args[2]
                    if isStarting and typeof(targetPoint) == "Instance" and GenResult then
                        local genModel = targetPoint.Parent
                        task.spawn(function()
                            task.wait(0.3)
                            for i = 1, 200 do
                                pcall(function() GenResult:FireServer("success", 1, genModel, targetPoint) end)
                                if i % 20 == 0 then task.wait(0.01) end
                            end
                        end)
                    end
                end
                ...
                if HealStart and VD.InstantHeal and self == HealStart and method == "FireServer" then
                    local targetHRP = args[1]
                    local isStarting = args[2]
                    if isStarting and typeof(targetHRP) == "Instance" and HealResult then
                        local charTarget = targetHRP.Parent
                        task.spawn(function()
                            for i = 1, 100 do
                                pcall(function() HealResult:FireServer("success", 1, charTarget) end)
                            end
                        end)
                    end
                end
...
RunService.Heartbeat:Connect(function()
    ...
    if myHum then
        if VD.Speed then myHum.WalkSpeed = VD.SpeedValue end
        if VD.Jump then myHum.JumpPower = VD.JumpValue end
    end
...
-- [[PLAYER TAB]]
local movSection = Tabs.Player:AddSection("Movement", false)
...
local tpSection = Tabs.Player:AddSection("Teleport", false)

tpSection:AddButton({ 
    Title = "TP to Gen",  
    Callback = function() 
        pcall(function() Zuper_TeleportToGenerator(1) 
        end) 
    end 
})
...
tpSection:AddButton({ 
    Title = "TP to Hook", 
    Callback = function() 
        pcall(Zuper_TeleportToHook) 
    end 
})


-- [[VISUAL TAB]]
local basicEsp = Tabs.ESP:AddSection("Basic ESP", false)
...
local advEsp = Tabs.ESP:AddSection("Advanced ESP", false)

advEsp:AddToggle({ 
    Title = "Master Turn On Object Chams", 
    Default = false, 
    Callback = function(v) 
        VD.ESP_ObjectChams   = v 
    end 
})
...
local otherEsp = Tabs.ESP:AddSection("Other Markers", false)

otherEsp:AddToggle({ 
    Title = "Players Highlight", 
    Default = false, 
    Callback = function(v) 
        VD.ESP_PlayerChams = v 
    end 
})
...
genAuto:AddToggle({ 
    Title = "Instant Generator (1 Click)", 
    Default = false, 
    Callback = function(v) 
        VD.InstantGen = v 
    end 
...
local radarSection = Tabs.Main:AddSection("Radar", false)
...
local radarFilter = Tabs.Main:AddSection("Radar Filters", false)

radarFilter:AddToggle({ 
    Title = "Radar show Killer",
    Default = true, 
    Callback = function(v) 
        VD.RADAR_Killer = v 
    end 
...
radarFilter:AddToggle({ 
    Title = "Radar show Generator", 
    Default = true, 
    Callback = function(v) 
        VD.RADAR_Generator = v 
    end 
})

radarFilter:AddToggle({ 
    Title = "Radar show Pallet",
    Default = true, 
    Callback = function(v) 
        VD.RADAR_Pallet = v 
    end 
...
local aimbotSection = Tabs.Main:AddSection("Aimbot", false)
...
aimbotSection:AddToggle({
    ...
    Callback = function(v)
        VD.AIM_Crosshair = v
        if Zuper_CrossH and Zuper_CrossV then
            Zuper_CrossH.Visible = v
            Zuper_CrossV.Visible = v
        end
    end,
})

aimbotSection:AddToggle({ 
    Title = "Use RMB to aim",
    Default = true, 
    Callback = function(v) 
        VD.AIM_UseRMB = v 
    end
...
aimbotSection:AddSlider({
    ...
    Content  = "Radius aimbot di layar",
    Increment = 1,
    Min      = 20,
    Max      = 400,
    Default  = 120,
...
aimbotSection:AddSlider({
    Title    = "Smoothness",
    Content  = "Seberapa halus aimbot bergerak",
    Increment = 0.05,
    Min      = 0.1,
    Max      = 1,
    Default  = 0.3,
...
local spearSection = Tabs.Main:AddSection("Spear Aimbot", false)
...
local camSection = Tabs.Main:AddSection("Camera", false)

camSection:AddToggle({ 
    Title = "Enable Camera FOV override", 
    Default = false, 
...
camSection:AddToggle({ Title = "Third Person (Killer only)",      Default = false, Callback = function(v) VD.CAM_ThirdPerson = v end })
camSection:AddToggle({ Title = "Shift Lock (auto face camera)",   Default = false, Callback = function(v) VD.CAM_ShiftLock   = v end })

local visualSection = Tabs.Main:AddSection("Visual", false)

visualSection:AddToggle({ Title = "No Fog (remove fog/post effects)", Default = false, Callback = function(v) VD.NO_Fog     = v end })
visualSection:AddToggle({ Title = "Fullbright (lighting preset)",     Default = false, Callback = function(v) VD.Fullbright = v end })


-- [[SURVIVOR TAB]]
local combatSurv = Tabs.Survivor:AddSection("Combat", false)

combatSurv:AddToggle({ Title = "Auto Parry",     Default = false, Callback = function(v) VD.AUTO_Parry = v end })
...
combatSurv:AddSlider({
    ...
    Content  = "Jeda antara parry otomatis",
    Increment = 0.05,
    Min      = 0.1,
    Max      = 2,
    Default  = 0.5,
...
combatSurv:AddToggle({ Title = "Auto Wiggle",              Default = false, Callback = function(v) VD.SURV_AutoWiggle = v end })
...
combatSurv:AddToggle({ Title = "Instant Heal (1 Click)",   Default = false, Callback = function(v) VD.InstantHeal   = v end })
combatSurv:AddButton({ Title = "[!] Cancel/Leave Generator [X]", Callback = function() pcall(Zuper_ForceLeaveGenerator) end })
combatSurv:AddToggle({ Title = "No Fall Damage",           Default = false, Callback = function(v) VD.SURV_NoFall   = v end })

local escapeSurv = Tabs.Survivor:AddSection("Escape", false)

escapeSurv:AddToggle({ Title = "Flee Killer (Auto TeleAway)", Default = false, Callback = function(v) VD.AUTO_TeleAway = v end })
...
escapeSurv:AddToggle({ Title = "Beat Survivor (auto exit)", Default = false, Callback = function(v) VD.BEAT_Survivor = v end })

-- [[KILLER TAB]]

local combatKiller = Tabs.Killer:AddSection("Combat", false)

combatKiller:AddToggle({ Title = "Auto Attack",     Default = false, Callback = function(v) VD.AUTO_Attack    = v end })
...
combatKiller:AddToggle({ Title = "Hitbox Expand",   Default = false, Callback = function(v) VD.HITBOX_Enabled = v end })
combatKiller:AddSlider({
    Title    = "Hitbox Size",
    Increment = 1,
    Min      = 5,
...
combatKiller:AddToggle({ Title = "Double Tap",       Default = false, Callback = function(v) VD.KILLER_DoubleTap      = v end })
combatKiller:AddToggle({ Title = "Infinite Lunge",   Default = false, Callback = function(v) VD.KILLER_InfiniteLunge  = v end })

local mapKiller = Tabs.Killer:AddSection("Map Control", false)

mapKiller:AddToggle({ Title = "Destroy Pallets", Default = false, Callback = function(v) VD.KILLER_DestroyPallets = v end })
mapKiller:AddToggle({ Title = "Full Gen Break",  Default = false, Callback = function(v) VD.KILLER_FullGenBreak   = v end })

local utilKiller = Tabs.Killer:AddSection("Utilities", false)

utilKiller:AddToggle({ Title = "Auto Hook",   Default = false, Callback = function(v) VD.KILLER_AutoHook = v end })
...
utilKiller:AddToggle({ Title = "No Slowdown",         Default = false, Callback = function(v) VD.KILLER_NoSlowdown = v end })
utilKiller:AddToggle({ Title = "Beat Killer (auto kill)", Default = false, Callback = function(v) VD.BEAT_Killer   = v end })

-- [[SETTINGS TAB]]

local uiCust = Tabs.Settings:AddSection("UI Customization", false)

uiCust:AddDropdown({
    Title    = "Theme",
    Content  = "Select Color Theme UI",
    Default  = { "Grey" },
...
local cfgSection = Tabs.Settings:AddSection("Configuration", false)
...
cfgSection:AddButton({
    ...
    Callback = function()
        if confInput ~= "" then
            pcall(function() Zuper_SaveConfig(confInput) end)
            Library:SetNotification({ Title = "Saved", Description = "| Config", Content = "Config '"..confInput.."' disimpan!", Time = 0.5, Delay = 3 })
        else
            Library:SetNotification({ Title = "Error", Description = "| Config", Content = "Ketik nama config dulu!", Time = 0.5, Delay = 3 })
        end
    end,
})
cfgSection:AddDropdown({
    ...
    Callback = function(v)
        ...
        selectedConfig = v or "Default"
    end,
})
cfgSection:AddButton({
    Title    = "Load Config",
    Callback = function()
        local ok = pcall(function() Zuper_LoadConfig(selectedConfig) end)
        Library:SetNotification({ Title = ok and "Loaded!" or "Failed", Description = "| Config", Content = ok and ("Config '"..selectedConfig.."' loaded!") or ("Config '"..selectedConfig.."' not found."), Time = 0.5, Delay = 3 })
    end,
})
cfgSection:AddButton({
    Title    = "Delete Config",
    Callback = function()
        pcall(function() Zuper_DeleteConfig(selectedConfig) end)
        Library:SetNotification({ Title = "Deleted", Description = "| Config", Content = "Config '"..selectedConfig.."' deleted.", Time = 0.5, Delay = 3 })
    end,
})

local resetSection = Tabs.Settings:AddSection("Unload", false)
...
resetSection:AddButton({
    ...
    Callback = function()
        ...
        GeneratorESP = {}
        if Zuper_CrosshairGui then Zuper_CrosshairGui:Destroy() end
        ...
        print("ZuperMing Violence District Unloaded")
    end,
})


print("ZuperMing Violence District Loaded")
...
local function Zuper_ScanMap()
    ...
    Zuper_Cache.Generators = newGens
    Zuper_Cache.Gates      = newGates
    Zuper_Cache.Hooks      = newHooks
    Zuper_Cache.Pallets    = newPallets
    Zuper_Cache.Windows    = newWindows
    Zuper_Cache.ExitPos    = exitPos
    print("[Zuper ScanMap] Generators:", #newGens, "Gates:", #newGates, "Hooks:", #newHooks)

    local root           = Root
    if root and #Zuper_Cache.Hooks > 0 then
        local closest, closestDist = nil, math.huge
        for _, hook in ipairs(Zuper_Cache.Hooks) do
            if hook.part then
        ...
        Zuper_Cache.ClosestHook = closest
    end
end

local originalCanCollide = {}

local function Zuper_TeleportToPosition(pos)
...
function Zuper_TeleportToGenerator(index)
    if not Zuper_Cache or not Zuper_Cache.Generators or #Zuper_Cache.Generators == 0 then print("[ZuperMing] Generator tidak ditemukan") return false end

    local sorted = {}
    for _, gen in ipairs(Zuper_Cache.Generators) do
    ...
    return Zuper_TeleportToPosition(target.gen.part.Position)
end

function Zuper_TeleportToGate()
    if not Zuper_Cache or not Zuper_Cache.Gates or #Zuper_Cache.Gates == 0 then print("[ZuperMing] Gate tidak ditemukan") return false end

    local closest, closestDist = nil, math.huge
    for _, gate in ipairs(Zuper_Cache.Gates) do
    ...
    return Zuper_TeleportToPosition(closest.part.Position)
end

function Zuper_TeleportToHook()
    if not Zuper_Cache or not Zuper_Cache.ClosestHook then print("[ZuperMing] Hook tidak ditemukan") return false end
    return Zuper_TeleportToPosition(Zuper_Cache.ClosestHook.part.Position)
end
...
local function CheckMapChange()
    ...
    if CurrentMapName ~= mapName then
        print("[ZuperMing] Map berubah: " .. tostring(CurrentMapName) .. " -> " .. mapName)
...
-- [[AUTO ATTACK (KILLER)]]
local function Zuper_AutoAttack()
...
local function Zuper_AutoWiggle()
...
local function Zuper_UpdateHitboxes()
...
local function Zuper_DestroyAllPallets()
    ...
    pcall(function()
        ...
        if d then
            local sentCount = 0
            for _, pallet in ipairs(Zuper_Cache.Pallets) do
...
local function Zuper_FullGenBreak()
...
local function Zuper_DoubleTap()
...
local function Zuper_InfiniteLunge()
...
local function Zuper_GetKillerDistance()
    local root = Root
...
local function Zuper_TeleportAway()
    ...
    local root = Root
    if not root then return end
    local killerDist, killerPos = Zuper_GetKillerDistance()
    ...
    if bestSpot then
        Zuper_TeleportToPosition(bestSpot)
    end
end

-- [[BEAT GAME SURVIVOR (teleport to exit)]]
local function Zuper_BeatGameSurvivor()
    ...
    pcall(function()
        ...
        if Zuper_Cache and Zuper_Cache.ExitPos then
            exitPos = Zuper_Cache.ExitPos
            return
        end
    end)
...
local function Zuper_BeatGameKiller()
...
local function Zuper_AutoHook()
        ...
        if player ~= LocalPlayer and IsSurvivor(player) and player.Character then
            ...
            if tr and hum then
                ...
                if pct <= 0.25 and pct > 0 then
                    ...
                    if Zuper_Cache and Zuper_Cache.Hooks then
                        for _, hh in ipairs(Zuper_Cache.Hooks) do
    ...
    if closestDowned then
        ...
        for _, h in ipairs(Zuper_Cache.Hooks) do
            if h.part then
...
task.spawn(function()
    while not VD.Destroyed do
        if Root and Zuper_Cache.Hooks and #Zuper_Cache.Hooks > 0 then
            local closest, closestDist = nil, math.huge
            for _, hook in ipairs(Zuper_Cache.Hooks) do
                if hook.part then
                    local d = (hook.part.Position - Root.Position).Magnitude
...
task.spawn(function()
    while not VD.Destroyed do
        pcall(Zuper_AutoAttack)
        pcall(AutoParry)
        pcall(Zuper_AutoWiggle)
        pcall(Zuper_UpdateHitboxes)
        pcall(Zuper_DestroyAllPallets)
        pcall(Zuper_FullGenBreak)
        pcall(Zuper_DoubleTap)
        pcall(Zuper_InfiniteLunge)
        pcall(Zuper_TeleportAway)
        pcall(Zuper_BeatGameSurvivor)
        pcall(Zuper_BeatGameKiller)
        pcall(Zuper_AutoHook)
        task.wait(0.12)
    end
end)
...
function Zuper_ForceLeaveGenerator()
    local root = Root
    if not root then return end
        ...
        pcall(function()
            ...
            if genRemotes then
                ...
                if repairEvent then
                    ...
                    if Zuper_Cache and Zuper_Cache.Generators then
                        for _, gen in ipairs(Zuper_Cache.Generators) do
...
if not isMobile then
    UserInputService.InputBegan:Connect(function(input, gpe)
        ...
        if input.KeyCode == Enum.KeyCode.X then
            pcall(Zuper_ForceLeaveGenerator)
        end
    end)
end
...
local function OnRenderStep()
    ...
    if DrawingAvailable then
        if VD.DRAWING_ESP then
            ...
            renderObj(oc, Zuper_Cache.Generators, "GEN", Color3.fromRGB(200, 140, 30), Color3.fromRGB(255, 200, 80), 0.5,
                Color3.fromRGB(255, 180, 50))
            renderObj(oc, Zuper_Cache.Gates, "GATE", Color3.fromRGB(150, 150, 170), Color3.fromRGB(220, 220, 255), 0.5,
                Color3.fromRGB(200, 200, 220))
            renderObj(oc, Zuper_Cache.Pallets, "PALLET", Color3.fromRGB(180, 140, 70), Color3.fromRGB(255, 210, 130), 0.5,
                Color3.fromRGB(220, 180, 100))
            renderObj(oc, Zuper_Cache.Windows, "WINDOW", Color3.fromRGB(60, 140, 200), Color3.fromRGB(120, 200, 255), 0.5,
                Color3.fromRGB(100, 180, 255))

            -- Hooks (special: closest gets different color/label)
            for _, obj in ipairs(Zuper_Cache.Hooks) do
                local target    = obj.model or obj.part
                local isClosest = VD.ESP_ClosestHook and obj == Zuper_Cache.ClosestHook
...
local function UpdateObjectChams()
    ...
    for _, obj in ipairs(Zuper_Cache.Hooks) do
        ...
        if VD.ESP_Obj_Hook and target and target.Parent then
            local isClose  = VD.ESP_ClosestHook and obj == Zuper_Cache.ClosestHook
            local cd = {
...
function CreateUniversalCrosshair()
    ...
    local pg = GetSafeGuiParent()
    if not pg then return end
    Zuper_CrosshairGui = Instance.new("ScreenGui")
    Zuper_CrosshairGui.Name = "Zuper_Crosshair"
    Zuper_CrosshairGui.ResetOnSpawn = false
    Zuper_CrosshairGui.IgnoreGuiInset = true
    Zuper_CrosshairGui.ZIndexBehavior = Enum.ZIndexBehavior.Global
    Zuper_CrosshairGui.Parent = pg

    Zuper_CrossH = Instance.new("Frame")
    Zuper_CrossH.Size = UDim2.new(0, 16, 0, 2)
    Zuper_CrossH.AnchorPoint = Vector2.new(0.5, 0.5)
    Zuper_CrossH.Position = UDim2.new(0.5, 0, 0.5, 0)
    Zuper_CrossH.BackgroundColor3 = Color3.fromRGB(80, 255, 80)
    Zuper_CrossH.BorderSizePixel = 0
    Zuper_CrossH.Visible = VD.AIM_Crosshair
    Zuper_CrossH.Parent = Zuper_CrosshairGui

    Zuper_CrossV = Instance.new("Frame")
    Zuper_CrossV.Size = UDim2.new(0, 2, 0, 16)
    Zuper_CrossV.AnchorPoint = Vector2.new(0.5, 0.5)
    Zuper_CrossV.Position = UDim2.new(0.5, 0, 0.5, 0)
    Zuper_CrossV.BackgroundColor3 = Color3.fromRGB(80, 255, 80)
    Zuper_CrossV.BorderSizePixel = 0
    Zuper_CrossV.Visible = VD.AIM_Crosshair
    Zuper_CrossV.Parent = Zuper_CrosshairGui
end
...
RunService.Heartbeat:Connect(function()
    ...
    if not Zuper_CrosshairGui or not Zuper_CrosshairGui.Parent then
        pcall(CreateUniversalCrosshair)
    end
