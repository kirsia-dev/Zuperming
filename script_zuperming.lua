...
ZuperMing:SetNotification({
	Title = "ZuperMing",
	Description = "| ZUPERMING",
	Content = "Script loaded successfully! /n Welcome to ZuperMing Hub.",
	Time = 0.5,
	Delay = 5,
})
...
local EconTab = Window:CreateTab({ Name = "Economy", Icon = "coins" })
local MgmtTab = Window:CreateTab({ Name = "Storage", Icon = "archive" })
local MiscTab = Window:CreateTab({ Name = "Misc", Icon = "wrench" })
local FinderTab = Window:CreateTab({ Name = "Finder", Icon = "search" })
local WebhookTab = Window:CreateTab({ Name = "Webhooks", Icon = "webhook" })
local ConfigTab = Window:CreateTab({ Name = "Settings", Icon = "settings" })
...
-- [FARM TAB]
do
	local HarvestGroup = FarmTab:AddSection("Harvesting")
	...
	local PlantGroup = FarmTab:AddSection("Planting")
	...
	local ShovelGroup = FarmTab:AddSection("Shoveling")
	...
	local SafetyGroup = FarmTab:AddSection("Movements")
	...
	local WaterGroup = FarmTab:AddSection("Watering")
	...
	local SprinklerGroup = FarmTab:AddSection("Sprinklers")
	...
	local StealGroup = FarmTab:AddSection("Auto Stealing")
	...
	StealGroup:AddButton({
		...
		Callback = function()
			if LP.Character and LP.Character:FindFirstChild("HumanoidRootPart") then
				...
				ZuperMing:SetNotification({ Title = "Auto Steal", Description = "| ZUPERMING", Content = "Garden position saved!", Time = 0.5, Delay = 3 })
			end
		end,
	})

	local CollectGroup = FarmTab:AddSection("Collecting")
	...
	local EspGroup = FarmTab:AddSection("Visuals")
	EspGroup:AddToggle({
		Title = "ESP Players",
		Default = false,
		Callback = function(s)
...
-- [ECON TAB]
do
	local SellGroup = EconTab:AddSection("Selling")
	...
	local BuyGroup = EconTab:AddSection("Shopping")
	...
	local GachaGroup = EconTab:AddSection("Gacha & Loot")
	...
	local AuctionGroup = EconTab:AddSection("Auction Sniper")
...
-- [MGMT TAB]
do
	local DoNGroup = MgmtTab:AddSection("Double Or Nothing")
	DoNGroup:AddSlider({
	...
	local InvGroup = MgmtTab:AddSection("Favorite / Unfavorite")
	...
	local MailGroup = MgmtTab:AddSection("Mailbox & Gifts")
	MailGroup:AddToggle({
...
-- [FINDER TAB]
do
	local FinderGroup = FinderTab:AddSection("Server & Pet Finder")
...
-- [WEBHOOK TAB]
do
	local WebGroup = WebhookTab:AddSection("Discord Webhook")
	WebGroup:AddInput({
		Title = "Webhook URL",
...
-- [MISC TAB]
do
	local MiscGroup = MiscTab:AddSection("Miscellaneous")
	MiscGroup:AddToggle({
		Title = "Anti-AFK",
		Default = true,
		Callback = function(s)
	...
	local ExploitGroup = MiscTab:AddSection("Exploits")
	...
	local UtilGroup = MiscTab:AddSection("Utility & Visuals")
	UtilGroup:AddSlider({
		Title = "WalkSpeed",
		Min = 16,
		Max = 200,
...
-- [SETTINGS TAB]
do
	local SettingsSection = ConfigTab:AddSection("Settings", true)

	SettingsSection:AddDropdown({
		Title = "Theme",
		Content = "Pilih warna tema UI",
		Default = {"Grey"},
		Options = ZuperMing:GetThemes(),
		Multi = false,
		Callback = function(Value)
			local selected = type(Value) == "table" and Value[1] or Value
			if selected and selected ~= "" then
				ZuperMing:SetTheme(selected)
			end
		end,
	})

	local SystemGroup = ConfigTab:AddSection("System")

	SystemGroup:AddButton({
		Title = "Unload Script",
		Callback = function()
			if ZuperMing and ZuperMing.Unload then
				ZuperMing:Unload()
			end
...
ZuperMing:SetNotification({ Title = "ZUPERMING", Description = "| ZUPERMING", Content = "Script loaded!", Time = 0.5, Delay = 5 })
