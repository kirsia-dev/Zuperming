...
-- Local variables (converted from _G)
local AcceptQuest
local AcientOne
local Addealer
local AimMethod
...
local AutoBigmom
local AutoBoss
local AutoBuyLawChip
local AutoChipBeli
local AutoChipFruit
local AutoClickLoop
local AutoColShad
local AutoCraftVolcanic
local AutoDeathStep
local AutoDragonTalon
local AutoDungeonFarm
local AutoEcBoss
local AutoEctoplasm
local AutoEvoRace
local AutoFMon
local AutoFactory
local AutoFarmChest
local AutoFarmGodChalice
local AutoFarmNear
local AutoFarmRaid
local AutoFarm_Bone
local AutoFarm_Cake
local AutoFireFlowers
local AutoFishingQuest
local AutoFistDarkness
local AutoGetQuestBounty
local AutoGetUsoap
local AutoHolyTorch
local AutoHopAdmin
local AutoHopServer
...
local AutoMatSoul
local AutoMiror
local AutoMysticIsland
local AutoNotifyFruit
local AutoPhoenixF
local AutoPoleV2
local AutoPressW
local AutoQuestComplete
local AutoRaidCastle
local AutoRipIngay
local AutoSaber
local AutoSaw
local AutoSellFish
local AutoSerpentBow
local AutoSkillZ
local AutoStartLawRaid
local AutoStartPrehistoric
local AutoStartRaid
local AutoTime
local AutoTridentW2
local AutoTyrant
...
local AutoZou
local Auto_Awakener
local Auto_Cake_Prince
local Auto_Cavender
local Auto_Def_DarkCoat
local Auto_Defense
local Auto_DevilFruit
local Auto_DonAcces
local Auto_Electric_Claw
local Auto_Fish
local Auto_God_Human
local Auto_Gun
local Auto_Human
local Auto_Melee
local Auto_Mink
local Auto_Rainbow_Haki
local Auto_SharkMan_Karate
local Auto_Skypiea
local Auto_Soul_Guitar
local Auto_StartRaid
local Auto_SuperHuman
local Auto_SwanGG
local Auto_Sword
local Auto_Tushita
local Auto_Yama
...
local ChooseWP
local CitizenQuest
local CollectPresent
local Collect_Ember
local Complete_Trials
local CraftVM
local CurrentTargetBoss
local DT_Uzoth
local DangerLV
local DangerSc
local DarkBladev3
...
local FarmBlazeEM
local FarmChestM
local FarmElfLevelCustom
local FarmEliteHunt
local FarmGodChalice
local FarmMastery_Dev
local FarmMastery_G
local FarmMastery_S
local FarmPriorityElf
local FastAttack
local FastAttackLoop1
local FastAttackLoop2
local FindBoss
local FindMirage
local FishBoat
local FrozenTP
...
local Prehis_DE
local Prehis_Find
local Prehis_KillGolem
local Prehis_Skills
local PrehistoricSkills
local RaceClickAutov3
local RaceClickAutov4
local Raiding
local Random_Auto
local Rdbone
local Relic123
local SailBoat_Hydra
local SailBoats
local SeaBeast1
local SelectChip
local SelectDN
...
G.Kill = function(I, e)
	...
	_tp(hrp.CFrame * CFrame.new(0, MobHeight, 0))
end
...
UseFruitSkills = function()
    ...
    if FruitSkills.Z then
        Useskills("Blox Fruit", "Z")
    end
    if FruitSkills.X then
        Useskills("Blox Fruit", "X")
    end
    if FruitSkills.C then
        Useskills("Blox Fruit", "C")
    end
    if FruitSkills.V then
        Useskills("Blox Fruit", "V")
    end
    if FruitSkills.F then
...
FarmPriorityElf = FarmPriorityElf or false
FarmMastery_S   = FarmMastery_S or false
...
local function FarmAtivo()
    if FarmPriorityElf or FarmElfLevelCustom then
        return true
    end

    if FarmMastery_S then
        return true
    end

    return StartFarm and (
        Level or
        AutoFarm_Bone or
        AutoFarm_Cake or
        FarmMastery_Dev or
        ...
        AutoTyrant or
        SailBoat_Hydra or WardenBoss or AutoFactory or HighestMirage or HCM or PGB or Leviathan1 or UPGDrago or Complete_Trials or TpDrago_Prehis or BuyDrago or AutoFireFlowers or DT_Uzoth or AutoBerry or Prehis_Find or Prehis_Skills or Prehis_DB or Prehis_DE or FarmBlazeEM or Dojoo or CollectPresent or AutoLawKak or TpLab or AutoPhoenixF or AutoFarmChest or AutoHytHallow or LongsWord or BlackSpikey or AutoHolyTorch or TrainDrago or AutoSaber or FarmMastery_Dev or CitizenQuest or AutoEctoplasm or KeysRen or Auto_Rainbow_Haki or obsFarm or AutoBigmom or Doughv2 or AuraBoss or Raiding or Auto_Cavender or TpPly or Bartilo_Quest or Level or FarmEliteHunt or AutoZou or AutoFarm_Bone or (getgenv()).AutoMaterial or CraftVM or FrozenTP or TPDoor or AcientOne or AutoFarmNear or AutoRaidCastle or DarkBladev3 or AutoFarmRaid or Auto_Cake_Prince or Addealer or TPNpc or TwinHook or FindMirage or FarmChestM or Shark or TerrorShark or Piranha or MobCrew or SeaBeast1 or FishBoat or Auto or AutoPoleV2 or Auto_SuperHuman or AutoDeathStep or Auto_SharkMan_Karate or Auto_Electric_Claw or AutoDragonTalon or Auto_Def_DarkCoat or Auto_God_Human or Auto_Tushita or AutoMatSoul or AutoKenVTWO or AutoSerpentBow or AutoFMon or Auto_Soul_Guitar or TPGEAR or AutoSaw or AutoTridentW2 or Auto_StartRaid or AutoEvoRace or AutoGetQuestBounty or MarinesCoat or TravelDres or Defeating or DummyMan or Auto_Yama or Auto_SwanGG or SwanCoat or AutoEcBoss or Auto_Mink or Auto_Human or Auto_Skypiea or Auto_Fish or CDK_TS or CDK_YM or CDK or AutoFarmGodChalice or AutoFistDarkness or AutoMiror or Teleport or AutoKilo or AutoGetUsoap or Praying or TryLucky or AutoColShad or AutoUnHaki or Auto_DonAcces or AutoRipIngay or DragoV3 or DragoV1 or SailBoats or NextIs or FarmGodChalice or IceBossRen or senth or senth2 or Lvthan or beasthunter or DangerLV or Relic123 or tweenKitsune or Collect_Ember or AutofindKitIs or snaguine or TwFruits or tweenKitShrine or Tp_LgS or Tp_MasterA or tweenShrine or FarmMastery_G or FarmMastery_S
    )
end
		...
		if tostring(I) == "FireServer" then
			if tostring(e[1]) == "RemoteEvent" then
				if tostring(e[2]) ~= "true" and tostring(e[2]) ~= "false" then
					if FarmMastery_G and not SoulGuitar or FarmMastery_Dev or FarmBlazeEM or Prehis_Skills or SeaBeast1 or FishBoat or PGB or Leviathan1 or Complete_Trials or AimMethod and ABmethod == "AimBots Skill" or AimMethod and ABmethod == "Auto Aimbots" then
						e[2] = MousePos;
						return i(unpack(e));
					end;
				end;
...
spawn(function()
	while task.wait() do
		pcall(function()
			if SailBoat_Hydra or WardenBoss or AutoFactory or HighestMirage or HCM or PGB or Leviathan1 or UPGDrago or Complete_Trials or TpDrago_Prehis or BuyDrago or AutoFireFlowers or DT_Uzoth or AutoBerry or Prehis_Find or Prehis_Skills or Prehis_DB or Prehis_DE or FarmBlazeEM or Dojoo or CollectPresent or AutoLawKak or TpLab or AutoPhoenixF or AutoFarmChest or AutoHytHallow or LongsWord or BlackSpikey or AutoHolyTorch or TrainDrago or AutoSaber or FarmMastery_Dev or CitizenQuest or AutoEctoplasm or KeysRen or Auto_Rainbow_Haki or obsFarm or AutoBigmom or Doughv2 or AuraBoss or Raiding or Auto_Cavender or TpPly or Bartilo_Quest or Level or FarmEliteHunt or AutoZou or AutoFarm_Bone or (getgenv()).AutoMaterial or CraftVM or FrozenTP or TPDoor or AcientOne or AutoFarmNear or AutoRaidCastle or DarkBladev3 or AutoFarmRaid or Auto_Cake_Prince or Addealer or TPNpc or TwinHook or FindMirage or FarmChestM or Shark or TerrorShark or Piranha or MobCrew or SeaBeast1 or FishBoat or Auto or AutoPoleV2 or Auto_SuperHuman or AutoDeathStep or Auto_SharkMan_Karate or Auto_Electric_Claw or AutoDragonTalon or Auto_Def_DarkCoat or Auto_God_Human or Auto_Tushita or AutoMatSoul or AutoKenVTWO or AutoSerpentBow or AutoFMon or Auto_Soul_Guitar or TPGEAR or AutoSaw or AutoTridentW2 or Auto_StartRaid or AutoEvoRace or AutoGetQuestBounty or MarinesCoat or TravelDres or Defeating or DummyMan or Auto_Yama or Auto_SwanGG or SwanCoat or AutoEcBoss or Auto_Mink or Auto_Human or Auto_Skypiea or Auto_Fish or CDK_TS or CDK_YM or CDK or AutoFarmGodChalice or AutoFistDarkness or AutoMiror or Teleport or AutoKilo or AutoGetUsoap or Praying or TryLucky or AutoColShad or AutoUnHaki or Auto_DonAcces or AutoRipIngay or DragoV3 or DragoV1 or SailBoats or NextIs or FarmGodChalice or IceBossRen or senth or senth2 or Lvthan or beasthunter or DangerLV or Relic123 or tweenKitsune or Collect_Ember or AutofindKitIs or snaguine or TwFruits or tweenKitShrine or Tp_LgS or Tp_MasterA or tweenShrine or FarmMastery_G or FarmMastery_S then
				shouldTween = true;
...
QuestB = function()
		if World1 then
			...
			elseif FindBoss == "Mob Leader" then
				bMon = "Mob Leader";
				PosB = CFrame.new(-2844.7307128906, 7.4180502891541, 5356.6723632813);
			elseif FindBoss == "Vice Admiral" then
				bMon = "Vice Admiral";
				Qname = "MarineQuest2";
				Qdata = 2;
			...
			elseif FindBoss == "Saber Expert" then
				bMon = "Saber Expert";
				PosB = CFrame.new(-1458.89502, 29.8870335, -50.633564);
			elseif FindBoss == "Warden" then
				bMon = "Warden";
				Qname = "ImpelQuest";
				Qdata = 1;
			...
			elseif FindBoss == "Chief Warden" then
				bMon = "Chief Warden";
				Qname = "ImpelQuest";
				Qdata = 2;
			...
			elseif FindBoss == "Fishman Lord" then
				bMon = "Fishman Lord";
				Qname = "FishmanQuest";
				Qdata = 3;
			...
			elseif FindBoss == "Thunder God" then
				bMon = "Thunder God";
				Qname = "SkyExp2Quest";
				Qdata = 3;
			...
			elseif FindBoss == "Ice Admiral" then
				bMon = "Ice Admiral";
				Qdata = nil;
			...
			elseif FindBoss == "Greybeard" then
				bMon = "Greybeard";
				Qdata = nil;
		...
		if World2 then
			...
			elseif FindBoss == "Don Swan" then
				bMon = "Don Swan";
				PosB = CFrame.new(2286.2004394531, 15.177839279175, 863.8388671875);
			elseif FindBoss == "Smoke Admiral" then
				bMon = "Smoke Admiral";
				Qname = "IceSideQuest";
				Qdata = 3;
			...
			elseif FindBoss == "Awakened Ice Admiral" then
			...
			elseif FindBoss == "Tide Keeper" then
				bMon = "Tide Keeper";
			...
			elseif FindBoss == "Darkbeard" then
				bMon = "Darkbeard";
				Qdata = nil;
		...
		if World3 then
			...
			elseif FindBoss == "Hydra Leader" then
				bMon = "Hydra Leader";
				Qname = "AmazonQuest2";
				Qdata = 3;
			...
			elseif FindBoss == "Kilo Admiral" then
				bMon = "Kilo Admiral";
			...
			elseif FindBoss == "Captain Elephant" then
			...
			elseif FindBoss == "Beautiful Pirate" then
			...
			elseif FindBoss == "Cake Queen" then
				bMon = "Cake Queen";
			...
			elseif FindBoss == "Soul Reaper" then
				bMon = "Soul Reaper";
				Qdata = nil;
...
QuestCheck = function()
    ...
    if World1 then
        ...
        elseif I >= 375 and I <= 399 then
            ...
            if Level and (PosQ.Position - game.Players.LocalPlayer.Character.HumanoidRootPart.Position).Magnitude > 10000 then
    ...
    elseif World2 then
        ...
        elseif I >= 1250 and I <= 1274 then
            ...
            if Level and (PosQ.Position - game.Players.LocalPlayer.Character.HumanoidRootPart.Position).Magnitude > 500 then
...
elseif I == 1650 or I <= 1699 then
    ...
    PosM = CFrame.new(4674.92676, 1134.82654, 996.308838, 0.731321394, -0, -0.682033002, 0, 1, -0, 0.682033002, 0, 0.731321394)
		...
		elseif I >= 2600 and I <= 2624 then
			...
			if (getgenv().AutoFarm or Level) and (PosQ.Position - game.Players.LocalPlayer.Character.HumanoidRootPart.Position).Magnitude > 10000 then
...
Settings.FastAttackMode = "Fast Attack"
...
local function StopAllAttacks()
    if FastAttackLoop1 then
        task.cancel(FastAttackLoop1)
        FastAttackLoop1 = nil
    end

    if FastAttackLoop2 then
        task.cancel(FastAttackLoop2)
        FastAttackLoop2 = nil
    end
...
FastAttack = false
if rz_FastAttack then rz_FastAttack = nil end
...
local Tabs = {
    Farming        = MainWindow:CreateTab({ Name = "Farming", Icon = "swords" })
}
...
local _s_Farming_1 = Tabs.Farming:AddSection("Auto Farm main", true)
_s_Farming_1:AddDropdown({
    Title = "Select Weapon",
...
_s_Farming_1:AddToggle({
    ...
    Callback = function(v)
        ...
        if v then
            if SelectedFarmMode == "Level" then
                Level = true
            elseif SelectedFarmMode == "Bone" then
                AutoFarm_Bone = true
            elseif SelectedFarmMode == "Cake Prince" then
                AutoFarm_Cake = true
            elseif SelectedFarmMode == "Tyrant Of The Skies" then
                AutoTyrant = true
            end
        end
...
spawn(function()
    game:GetService("RunService").Stepped:Connect(function()
        if AutoTyrant and StartFarm then
        ...
        if AutoTyrant and StartFarm then
            pcall(function()
                ...
                if boss and boss.Humanoid.Health > 0 then
                    ...
                    until not boss.Parent or boss.Humanoid.Health <= 0 or not StartFarm or not AutoTyrant
                ...
                elseif eyes == 4 then
                    ...
                    if AcceptQuest and not hasQuest then
                        local char = plr.Character
                        if char and char:FindFirstChild("HumanoidRootPart") then
...
spawn(function()
    while task.wait() do
        if AutoFarm_Bone and StartFarm then
            pcall(function()
                local plr = game.Players.LocalPlayer
                ...
                if AcceptQuest and not QuestUI.Visible then
    ...
    while task.wait() do
        if Level and StartFarm then
            pcall(function()
                ...
                repeat task.wait() G.Kill(CurrentMob, true) until not StartFarm or not Level or not CurrentMob.Parent or CurrentMob.Humanoid.Health <= 0
                CurrentMob = nil
            end)
        end
    end
...
spawn(function()
    while task.wait() do
        if AutoFarm_Cake and StartFarm then
            pcall(function()
                local plr = game.Players.LocalPlayer
                local hrp = plr.Character and plr.Character:FindFirstChild("HumanoidRootPart")
                if not hrp then return end
                ...
                if AcceptQuest and not plr.PlayerGui.Main.Quest.Visible then
                ...
                if bestMob then
                    ...
                    until not StartFarm or not AutoFarm_Cake or not bestMob.Parent or bestMob.Humanoid.Health <= 0
                else
...
local _s_Farming_2 = Tabs.Farming:AddSection("Other", true)
MaxFarmDistance = 325

local Players = game:GetService("Players")
local ReplicatedStorage = game:GetService("ReplicatedStorage")

_s_Farming_2:AddToggle({
...
spawn(function()
    while task.wait() do
        pcall(function()
            if not AutoFarmNear then return end
            ...
            if ClosestEnemy and (not MaxFarmDistance or ShortestDistance <= MaxFarmDistance) then
                repeat
                    task.wait()
                    if G and G.Kill then
                        G.Kill(ClosestEnemy, AutoFarmNear)
                    end
                until not AutoFarmNear
...
spawn(function()
    ...
    while task.wait(Sec) do
        pcall(function()
            if not AutoFactory then return end
            ...
            if Core and Core:FindFirstChild("Humanoid") and Core.Humanoid.Health > 0 then
                repeat
                    ...
                    if not AutoFactory then break end
                ...
                until Core.Humanoid.Health <= 0 or AutoFactory == false
...
local _s_Farming_3 = Tabs.Farming:AddSection("Collect", true)
_s_Farming_3:AddToggle({
...
local _s_Farming_4 = Tabs.Farming:AddSection("Material", true)
_s_Farming_4:AddDropdown({
...
if World3 then
local _s_Farming_5 = Tabs.Farming:AddSection("Bones", true)

_s_Farming_5:AddToggle({
    Title = "Trade Bone",
    Default = false,
    Callback = function(Value)
        Rdbone = Value
...
spawn(function()
    while wait(Sec) do
        if AutoHytHallow then
            pcall(function()
                local mob = GetConnectionEnemies("Soul Reaper")
                if mob then
                    repeat task.wait()
                        G.Kill(mob,AutoHytHallow)
                    until mob.Humanoid.Health <= 0 or not AutoHytHallow
                else
                    if not GetBP("Hallow Essence") then
                        repeat task.wait(.1)
                            replicated.Remotes.CommF_:InvokeServer("Bones","Buy",1,1)
                        until not AutoHytHallow or GetBP("Hallow Essence")
                    else
                        local pos = CFrame.new(-8932.32,146.83,6062.55)
                        repeat
                            task.wait()
                            _tp(pos)
                        until not AutoHytHallow or (plr.Character.HumanoidRootPart.Position - pos.Position).Magnitude <= 5
                ...
                if plr.Character and plr.Character:FindFirstChild("HumanoidRootPart") then
                    ...
                    if (hrp.Position - prayPos.Position).Magnitude > 5 then
                        repeat
                            task.wait()
                            _tp(prayPos)
                        until not Pray or (plr.Character.HumanoidRootPart.Position - prayPos.Position).Magnitude <= 5
                    end
                ...
                if plr.Character and plr.Character:FindFirstChild("HumanoidRootPart") then
                    ...
                    if (hrp.Position - luckPos.Position).Magnitude > 5 then
                        repeat
                            task.wait()
                            _tp(luckPos)
                        until not Trylux or (plr.Character.HumanoidRootPart.Position - luckPos.Position).Magnitude <= 5
                    end
...
FindBoss = FindBoss or (Boss and Boss[1])

local _s_Farming_6 = Tabs.Farming:AddSection("Boss Farm", true)

local BossStatus = _s_Farming_6:AddParagraph({
...
task.spawn(function()
    while task.wait(1) do
        pcall(function()
            if FindBoss and (game:GetService("ReplicatedStorage"):FindFirstChild(FindBoss) or
               (workspace.Enemies and workspace.Enemies:FindFirstChild(FindBoss)) or
               workspace:FindFirstChild(FindBoss)) then
            else
            end
        end)
    end
...
local BossDropdown = _s_Farming_6:AddDropdown({
    Title = "Select Boss",
    Options = Boss,
...
_s_Farming_6:AddToggle({
    Title = "Auto Farm Boss Select",
    Default = false,
    Callback = function(v)
        AutoBoss = v
        if v then FarmAllBoss = false end
    end
})
...
local function EquipWeaponBoss()
    ...
    if SelectWeapon then
        local tool = game.Players.LocalPlayer.Backpack:FindFirstChild(SelectWeapon) or char:FindFirstChild(SelectWeapon)
...
task.spawn(function()
    while task.wait() do
        if not FarmAllBoss then
            task.wait(0.5)
            continue
        end
        if CurrentTargetBoss and CurrentTargetBoss.Parent and CurrentTargetBoss:FindFirstChild("Humanoid") and CurrentTargetBoss.Humanoid.Health > 0 then
            task.wait(0.2)
            continue
        end
        local char = game.Players.LocalPlayer.Character
        ...
        if #potentialBosses > 0 then
            ...
            CurrentTargetBoss = potentialBosses[1].Model
        else
            CurrentTargetBoss = nil
        end
        task.wait(0.2)
...
task.spawn(function()
    while task.wait() do
        pcall(function()
            if AutoBoss and FindBoss then
                if QuestB then QuestB() end
                local NeedQuest = false
                local char = game.Players.LocalPlayer.Character
                if not char or not char:FindFirstChild("HumanoidRootPart") then return end
                if AutoAcceptQuest and Qname and Qdata and PosQBoss then
                ...
                local specificBoss = workspace:FindFirstChild("Enemies") and workspace.Enemies:FindFirstChild(FindBoss) or workspace:FindFirstChild(FindBoss)
            ...
            elseif FarmAllBoss then
                if CurrentTargetBoss then
                    GoToBoss(CurrentTargetBoss)
                end
            end
        end)
    end
...
task.spawn(function()
    while task.wait(0.5) do
        if game.Players.LocalPlayer.Backpack and ChooseWP then
            for _, e in pairs(game.Players.LocalPlayer.Backpack:GetChildren()) do
                if e.ToolTip == ChooseWP then SelectWeapon = e.Name end
            end
        end
    end
end)
...
if World3 then
local _s_Farming_7 = Tabs.Farming:AddSection("Farm Mastery", true)
...
FruitSkills = FruitSkills or { Z = false, X = false, C = false, V = false, F = false }

_s_Farming_7:AddToggle({ Title = "Use Skill Z (Fruit)", Default = false, Callback = function(v) FruitSkills.Z = v end })
_s_Farming_7:AddToggle({ Title = "Use Skill X (Fruit)", Default = false, Callback = function(v) FruitSkills.X = v end })
_s_Farming_7:AddToggle({ Title = "Use Skill C (Fruit)", Default = false, Callback = function(v) FruitSkills.C = v end })
_s_Farming_7:AddToggle({ Title = "Use Skill V (Fruit)", Default = false, Callback = function(v) FruitSkills.V = v end })
_s_Farming_7:AddToggle({ Title = "Use Skill F (Fruit)", Default = false, Callback = function(v) FruitSkills.F = v end })

local function UseFruitSkills()
    weaponSc("Blox Fruit")
    if FruitSkills.Z then Useskills("Blox Fruit", "Z") end
    if FruitSkills.X then Useskills("Blox Fruit", "X") end
    if FruitSkills.C then Useskills("Blox Fruit", "C") end
    if FruitSkills.V then Useskills("Blox Fruit", "V") end
    if FruitSkills.F then
        vim1:SendKeyEvent(true, "F", false, game)
        vim1:SendKeyEvent(false, "F", false, game)
    end
...
spawn(function()
    while task.wait(Sec) do
        if FarmMastery_Dev then
            pcall(function()
                local list = (SelectedIsland == "Cake" and CAKE_MOBS or BONE_MOBS)
                ...
                if mob then
                    ...
                    until not FarmMastery_Dev

                else
                    if SelectedIsland == "Cake" then
                        ...
                        if tonumber(item.Mastery) >= 1 and tonumber(item.Mastery) <= 599 then
                            if GetBP(swordName) then
                                if mob then
                                    ...
                                    until FarmMastery_S == false or not mob.Parent or mob.Humanoid.Health <= 0
                                else
                                    if SelectedIsland == "Cake" then
                                        _tp(CFrame.new(-1943.6765, 251.5095, -12337.8808))
                                    else
...
spawn(function()
	while wait(.2) do
		pcall(function()
			if RaceClickAutov3 then
				repeat
					replicated.Remotes.CommE:FireServer("ActivateAbility");
					wait(30);
				until not RaceClickAutov3;
			end;
		end);
	end;
end);
...
AutoHopAdmin = AutoHopAdmin or false
local AdminNames = {
...
spawn(function()
    ...
    runService.Stepped:Connect(function()
        pcall(function()
            ...
            elseif player.Character and not NoClip then
                for _, part in pairs(player.Character:GetDescendants()) do
                    if part:IsA("BasePart") then
...
AutoHopServer = AutoHopServer or false
HopDelay = HopDelay or (30 * 60)
    ...
    while task.wait(1) do
        if AutoHopServer then
            pcall(function()
                ...
                if tick() - HopTimer >= HopDelay then
                    HopTimer = tick()
                    Hop()
                end
            end)
                    ...
                    if plr.Data.Level.Value >= 700 then
                        ...
                        if IceDoor then
                            if IceDoor.CanCollide == true and IceDoor.Transparency == 0 then
                                ...
                                until not TravelDres or (Root.Position - CFrame.new(1347.7124, 37.3751602, -1325.6488).Position).Magnitude <= 5
                                    ...
                                    for _, e in pairs(Enemies:GetChildren()) do
                                        if e.Name == "Ice Admiral" and G.Alive(e) then
                                            repeat
                                                task.wait()
                                                G.Kill(e, TravelDres)
                                            until TravelDres == false or e.Humanoid.Health <= 0
                                ...
                                if string.find(QuestUI.Container.QuestTitle.Title.Text, "Swan Pirates") then
                                    ...
                                    if mob then
                                        repeat
                                            wait()
                                            G.Kill(mob, AutoZou)
                                        until not AutoZou or not mob.Parent or mob.Humanoid.Health <= 0 or not QuestUI.Visible
                                    else
                        ...
                        elseif bartiloProgress == 1 then
                            ...
                            if mob then
                                repeat
                                    wait()
                                    G.Kill(mob, AutoZou)
                                until not AutoZou or not mob.Parent or mob.Humanoid.Health <= 0
                            else
            ...
            if obsFarm then
                ...
                if targetEnemy then
                    if KenTest then
                        ...
                        until obsFarm == false or KenTest == false
                    else
                        repeat
                            wait()
                            _tp(targetEnemy.HumanoidRootPart.CFrame * CFrame.new(0, 50, 0))
                        until obsFarm == false or KenTest
                    end
                elseif farmPos then
                    _tp(farmPos)
                end
...
if World3 then
                ...
                pcall(function()
                    ...
                    if plr.PlayerGui.Main.Quest.Visible == true and string.find(plr.PlayerGui.Main.Quest.Container.QuestTitle.Title.Text, "Defeat 50 Forest Pirates") then
                        ...
                        if I then
                            repeat
                                wait();
                                G.Kill(I, AutoKenVTWO);
                            until not AutoKenVTWO or I.Humanoid.Health <= 0 or plr.PlayerGui.Main.Quest.Visible == false;
                        else
                            _tp(n);
                        end;
                    ...
                    if not plr.Backpack:FindFirstChild("Fruit Bowl") or not plr.Character:FindFirstChild("Fruit Bowl") then
                        ...
                        if plr.Backpack:FindFirstChild("Banana") and (plr.Backpack:FindFirstChild("Apple") and plr.Backpack:FindFirstChild("Pineapple")) or plr:FindFirstChild("Banana") and (plr:FindFirstChild("Apple") and plr:FindFirstChild("Pineapple")) then
                            repeat
                                wait();
                                _tp(I);
                            until AutoKenVTWO or plr.Character.HumanoidRootPart.CFrame == I;
                            replicated.Remotes.CommF_:InvokeServer("CitizenQuestProgress", "Citizen");
                        end;
                    ...
                    if Lv >= 1800 and (replicated.Remotes.CommF_:InvokeServer("CitizenQuestProgress")).KilledBandits == false then
                        if string.find(plr.PlayerGui.Main.Quest.Container.QuestTitle.Title.Text, "Forest Pirate") and (string.find(plr.PlayerGui.Main.Quest.Container.QuestTitle.Title.Text, "50") and plr.PlayerGui.Main.Quest.Visible == true) then
                            ...
                            if I then
                                repeat
                                    task.wait();
                                    G.Kill(I, CitizenQuest);
                                until CitizenQuest == false or not I.Parent or I.Humanoid.Health <= 0 or plr.PlayerGui.Main.Quest.Visible == false;
                            else
                        ...
                        if plr.PlayerGui.Main.Quest.Visible and (string.find(plr.PlayerGui.Main.Quest.Container.QuestTitle.Title.Text, "Captain Elephant") and plr.PlayerGui.Main.Quest.Visible == true) then
                            if I then
                                repeat
                                    task.wait();
                                    G.Kill(I, CitizenQuest);
                                until CitizenQuest == false or I.Humanoid.Health <= 0 or not I.Parent or plr.PlayerGui.Main.Quest.Visible == false;
                            else
                        ...
                        if string.find(plr.PlayerGui.Main.Quest.Container.QuestTitle.Title.Text, "Diablo") or string.find(plr.PlayerGui.Main.Quest.Container.QuestTitle.Title.Text, "Urban") or string.find(plr.PlayerGui.Main.Quest.Container.QuestTitle.Title.Text, "Deandre") then
                            ...
                            for I, e in pairs(Enemies:GetChildren()) do
                                if (string.find(e.Name, "Diablo") or string.find(e.Name, "Urban") or string.find(e.Name, "Deandre")) and G.Alive(e) then
                                    repeat
                                        wait();
                                        G.Kill(e, FarmEliteHunt);
                                    until not FarmEliteHunt or plr.PlayerGui.Main.Quest.Visible == false or not e.Parent or e.Humanoid.Health <= 0;
                                end;
                            end;
                        end;
                    else
    ...
    spawn(function()
        while wait(.2) do
            if StopWhenChalice and FarmEliteHunt then
                pcall(function()
                ...
                if Auto_Tushita then
                    if workspace.Map.Turtle:FindFirstChild("TushitaGate") then
                        if not GetBP("Holy Torch") then
                            ...
                            until not Auto_Tushita or ((CFrame.new(-10752, 417, -9366)).Position - plr.Character.HumanoidRootPart.Position).Magnitude <= 10;
                            wait(.7);
                            repeat
                                task.wait();
                                _tp(CFrame.new(-11672, 334, -9474));
                            until not Auto_Tushita or ((CFrame.new(-11672, 334, -9474)).Position - plr.Character.HumanoidRootPart.Position).Magnitude <= 10;
                            wait(.7);
                            repeat
                                task.wait();
                                _tp(CFrame.new(-12132, 521, -10655));
                            until not Auto_Tushita or ((CFrame.new(-12132, 521, -10655)).Position - plr.Character.HumanoidRootPart.Position).Magnitude <= 10;
                            wait(.7);
                            repeat
                                task.wait();
                                _tp(CFrame.new(-13336, 486, -6985));
                            until not Auto_Tushita or ((CFrame.new(-13336, 486, -6985)).Position - plr.Character.HumanoidRootPart.Position).Magnitude <= 10;
                            wait(.7);
                            repeat
                                task.wait();
                                _tp(CFrame.new(-13489, 332, -7925));
                            until not Auto_Tushita or ((CFrame.new(-13489, 332, -7925)).Position - plr.Character.HumanoidRootPart.Position).Magnitude <= 10;
                        end;
                    else
                        ...
                        if I then
                            repeat
                                task.wait();
                                G.Kill(I, Auto_Tushita);
                            until I.Humanoid.Health <= 0 or not Auto_Tushita or not I.Parent;
                        else
                        ...
                        if (workspace.Map.Waterfall.SealedKatana.Handle.Position - plr.Character.HumanoidRootPart.Position).Magnitude >= 20 then
                            ...
                            if I then
                                repeat
                                    wait();
                                    G.Kill(I, Auto_Yama);
                                until I.Humanoid.Health <= 0 or not I.Parent or not Auto_Yama;
...
if World2 then
    ...
    spawn(function()
        while wait(Sec) do
            pcall(function()
                if AutoTridentW2 then
                    local boss = GetConnectionEnemies("Tide Keeper")
                    if boss then
                        repeat
                            task.wait()
                            G.Kill(boss, AutoTridentW2)
                        until AutoTridentW2 == false or not boss.Parent or boss.Humanoid.Health <= 0
                    else
                ...
                if AutoSaw then
                    ...
                    if boss then
                        repeat
                            task.wait()
                            G.Kill(boss, AutoSaw)
                        until AutoSaw == false or boss.Humanoid.Health <= 0
                    else
            ...
            pcall(function()
                if SwanCoat then
                    ...
                    if boss then
                        repeat
                            wait()
                            G.Kill(boss, SwanCoat)
                        until SwanCoat == false or not boss.Parent or boss.Humanoid.Health <= 0
                    else
            ...
            pcall(function()
                if MarinesCoat then
                    ...
                    if boss then
                        repeat
                            wait()
                            G.Kill(boss, MarinesCoat)
                        until MarinesCoat == false or not boss.Parent or boss.Humanoid.Health <= 0
                    else
            ...
            pcall(function()
                if WardenBoss then
                    ...
                    if boss then
                        repeat
                            wait()
                            G.Kill(boss, WardenBoss)
                        until WardenBoss == false or not boss.Parent or boss.Humanoid.Health <= 0
                    else
            ...
            if AutoColShad then
                pcall(function()
                    ...
                    if boss then
                        repeat
                            task.wait()
                            G.Kill(boss, AutoColShad)
                        until AutoColShad == false or not boss.Parent or boss.Humanoid.Health <= 0
                    else
                ...
                if AutoEcBoss then
                    ...
                    elseif GetM("Ectoplasm") <= 99 then
                        ...
                        if boss then
                            repeat
                                wait()
                                G.Kill(boss, AutoEcBoss)
                            until not AutoEcBoss or not boss.Parent or boss.Humanoid.Health <= 0
                        else
            ...
            pcall(function()
                if IceBossRen then
                    ...
                    if boss then
                        repeat
                            task.wait()
                            G.Kill(boss, IceBossRen)
                        until IceBossRen == false or not boss.Parent or boss.Humanoid.Health <= 0
                    else
                ...
                if KeysRen then
                    if GetBP("Hidden Key") then
                        ...
                        if targetMob then
                            repeat
                                task.wait()
                                G.Kill(targetMob, KeysRen)
                            until GetBP("Hidden Key") or KeysRen == false or not targetMob.Parent or targetMob.Humanoid.Health <= 0
                        else
                ...
                pcall(function()
                    ...
                    elseif GetWP("Bisento") then
                        ...
                        if boss then
                            repeat
                                wait()
                                G.Kill(boss, Greybeard)
                            until Greybeard == false or not boss.Parent or boss.Humanoid.Health <= 0
                        else
                ...
                if CDK then
                    ...
                    if boss then
                        repeat
                            ...
                            G.Kill(boss, CDK)
                        until not CDK or not boss.Parent or boss.Humanoid.Health <= 0
                    else
            ...
            if Auto_SwanGG then
                pcall(function()
                    ...
                    if boss then
                        repeat
                            wait()
                            G.Kill(boss, Auto_SwanGG)
                        until Auto_SwanGG == false or not boss.Parent or boss.Humanoid.Health <= 0
                    else
            ...
            pcall(function()
                if Auto_Cavender then
                    ...
                    if boss then
                        repeat
                            wait()
                            G.Kill(boss, Auto_Cavender)
                        until not Auto_Cavender or boss.Humanoid.Health <= 0
                    else
            ...
            if AutoBigmom then
                pcall(function()
                    ...
                    if boss then
                        repeat
                            task.wait()
                            G.Kill(boss, AutoBigmom)
                        until not AutoBigmom or not boss.Parent or boss.Humanoid.Health <= 0
                    else
                ...
                pcall(function()
                    if plr.PlayerGui.Main.Quest.Visible == false then
                        ...
                        if dummy then
                            repeat
                                wait()
                                G.Kill(dummy, DummyMan)
                            until not DummyMan or not dummy.Parent or dummy.Humanoid.Health <= 0
                        else
...
AutoPressW = AutoPressW or false

NoClipShip = NoClipShip or false
        ...
        pcall(function()
            for _, boat in pairs(workspace.Boats:GetChildren()) do
                for _, part in pairs(boat:GetDescendants()) do
                    if part:IsA("BasePart") then
                        if NoClipShip or SailBoats or Prehis_Find or FindMirage then
                            part.CanCollide = false
                        else
                            part.CanCollide = true
                        end
                    ...
                    if not workspace._WorldOrigin.Locations:FindFirstChild("Kitsune Island", true) then
                        ...
                        if not MyBoat then
                            ...
                            if (BoatPos.Position - plr.Character.HumanoidRootPart.Position).Magnitude <= 10 then
                                replicated.Remotes.CommF_:InvokeServer("BuyBoat", SelectedBoat)
                            end
                        else
                            if plr.Character.Humanoid.Sit == false then
                                ...
                                until not AutofindKitIs or (TargetPos.Position - plr.Character.HumanoidRootPart.Position).Magnitude <= 10 or workspace._WorldOrigin.Locations:FindFirstChild("Kitsune Island") or plr.Character.Humanoid.Sit == false
                    ...
                    if Shrine then
                        for _, Part in next, Shrine:GetDescendants() do
                            if Part:IsA("BasePart") and Part.Name:find("NeonShrinePart") then
                                ...
                                until tweenShrine == false or not Island
                            end
                        end
                    else
                        _tp((workspace._WorldOrigin.Locations:FindFirstChild("Kitsune Island")).CFrame * CFrame.new(0, 500, 0))
...
PrehistoricSkills = PrehistoricSkills or {
...
local function UsePrehistoricSkills()
    if PrehistoricSkills.Melee.Z then Useskills("Melee", "Z") end
    if PrehistoricSkills.Melee.X then Useskills("Melee", "X") end
    if PrehistoricSkills.Melee.C then Useskills("Melee", "C") end

    task.wait(0.2)

    if PrehistoricSkills.Sword.Z then Useskills("Sword", "Z") end
    if PrehistoricSkills.Sword.X then Useskills("Sword", "X") end

    task.wait(0.2)

    if PrehistoricSkills["Blox Fruit"].Z then Useskills("Blox Fruit", "Z") end
    if PrehistoricSkills["Blox Fruit"].X then Useskills("Blox Fruit", "X") end
    if PrehistoricSkills["Blox Fruit"].C then Useskills("Blox Fruit", "C") end
    if PrehistoricSkills["Blox Fruit"].V then Useskills("Blox Fruit", "V") end
    if PrehistoricSkills["Blox Fruit"].F then
        vim1:SendKeyEvent(true, "F", false, game)
        vim1:SendKeyEvent(false, "F", false, game)
    end
    ...
    if PrehistoricSkills.Gun.Z then Useskills("Gun", "Z") end
    if PrehistoricSkills.Gun.X then Useskills("Gun", "X") end
end
                ...
                if not PrehistoricLoc then
                    ...
                    if not MyBoat then
                        ...
                        if (BoatPos.Position - hrp.Position).Magnitude <= 10 then
                            replicated.Remotes.CommF_:InvokeServer("BuyBoat", SelectedBoat or "Guardian")
                        end
                        return
                    end
        ...
        if Prehis_KillGolem then
            pcall(function()
                ...
                if golem and golem:FindFirstChild("Humanoid") then
                    ...
                    until not Prehis_KillGolem or not golem.Parent or golem.Humanoid.Health <= 0
                end
            end)
        end
    end
...
AutoCraftVolcanic = AutoCraftVolcanic or false

task.spawn(function()
    local CraftRemote = replicated.Modules.Net["RF/Craft"]
    while task.wait(30) do
        if AutoCraftVolcanic then
            pcall(function()
            ...
            pcall(function()
                if GetM("Dragon Egg") <= 0 then
                    repeat
                        ...
                        Prehis_Skills = true
                        Prehis_DE = true
                    until not DragoV1 or GetM("Dragon Egg") >= 1
                    Prehis_Find = false
                    Prehis_Skills = false
                    Prehis_DE = false
                end
        ...
        if AutoFireFlowers then
            pcall(function()
                ...
                if ForestPirate then
                    repeat
                        wait()
                        G.Kill(ForestPirate, AutoFireFlowers)
                    until not AutoFireFlowers or not ForestPirate.Parent or ForestPirate.Humanoid.Health <= 0 or FireFlowers
                else
                    _tp(CFrame.new(-13206.452148438, 425.89199829102, -7964.5537109375))
                end
                ...
                if workspace.Map:FindFirstChild("DracoTrial") then
                    ...
                    for _, spot in ipairs(RelicSpots) do
                        repeat
                            wait()
                            _tp(spot)
                        until not Relic123 or (spot.Position - Root.Position).Magnitude <= 10
                        wait(2.5)
                    end
                else
            ...
            pcall(function()
                ...
                elseif plr.Character:FindFirstChild("RaceTransformed") and plr.Character.RaceTransformed.Value == false then
                    ...
                    if mob then
                        repeat
                            wait()
                            G.Kill(mob, TrainDrago)
                        until TrainDrago == false or mob.Humanoid.Health <= 0 or not mob.Parent
                    else
                        _tp(CFrame.new(4620.6157226562, 1002.2954711914, 399.08688354492))
                    end
                end
                ...
                if questType and not backToDojo() then
                        ...
                        if mob then
                            repeat
                                ...
                                if mob and mob:FindFirstChild("HumanoidRootPart") then
                                    _tp(mob.HumanoidRootPart.CFrame * CFrame.new(0, 30, 0))
                                    G.Kill(mob, FarmBlazeEM)
                                end
                            until not FarmBlazeEM or not mob or not mob.Parent or mob.Humanoid.Health <= 0 or backToDojo()
                        else
                    ...
                    elseif questType == "destroy" then
                        ...
                        if Bamboo then
                            ...
                            until not FarmBlazeEM or not questType or backToDojo()
                        end
                    end
                else
                ...
                if not workspace._WorldOrigin.Locations:FindFirstChild("Mirage Island", true) then
                    ...
                    if not MyBoat then
                        ...
                        if plr.Character.Humanoid.Sit == false then
                            ...
                            until not FindMirage or (TargetPos.Position - plr.Character.HumanoidRootPart.Position).Magnitude <= 10 or workspace._WorldOrigin.Locations:FindFirstChild("Mirage Island") or plr.Character.Humanoid.Sit == false
                            plr.Character.Humanoid.Sit = false
                        end
                    end
                else
            ...
            pcall(function()
                ...
                elseif RaceTransformed and RaceTransformed.Value == false then
                    ...
                    if mob then
                        repeat
                            wait()
                            G.Kill(mob, AcientOne)
                        until AcientOne == false or mob.Humanoid.Health <= 0 or not mob.Parent
                    else
            ...
            pcall(function()
                ...
                elseif race == "Fishman" then
                    ...
                    if beast then
                        ...
                        until Complete_Trials == false or not beast or not beast.Parent or beast.Health.Value <= 0
                    end
                ...
                elseif race == "Human" or race == "Ghoul" then
                    ...
                    if mob then
                        repeat
                            wait()
                            G.Kill(mob, Complete_Trials)
                        until Complete_Trials == false or not mob.Parent or mob.Humanoid.Health <= 0
                    end
                end
            end)
        end
                    ...
                    elseif alchemist == 1 then
                        ...
                        elseif not GetBP("Flower 3") then
                            ...
                            if mob then
                                repeat
                                    wait()
                                    G.Kill(mob, Auto_Mink)
                                until GetBP("Flower 3") or not mob.Parent or mob.Humanoid.Health <= 0 or Auto_Mink == false
                            else
                    ...
                    elseif alchemist == 1 then
                        ...
                        elseif not GetBP("Flower 3") then
                            ...
                            if mob then
                                repeat
                                    wait()
                                    G.Kill(mob, Auto_Human)
                                until GetBP("Flower 3") or not mob.Parent or mob.Humanoid.Health <= 0 or Auto_Human == false
                            else
                                _tp(CFrame.new(980.09851074219, 121.33129882812, 1287.2093505859))
                            end
                        end
                ...
                elseif replicated.Remotes.CommF_:InvokeServer("Wenlocktoad", "1") == 1 then
                    ...
                    for _, bossName in ipairs(bosses) do
                        ...
                        if boss then
                            repeat
                                wait()
                                G.Kill(boss, Auto_Human)
                            until boss.Humanoid.Health <= 0 or not boss.Parent or not Auto_Human
                        else
                            local bossPositions = {
                    ...
                    elseif alchemist == 1 then
                        ...
                        elseif not GetBP("Flower 3") then
                            ...
                            if mob then
                                repeat
                                    wait()
                                    G.Kill(mob, Auto_Skypiea)
                                until GetBP("Flower 3") or not mob.Parent or mob.Humanoid.Health <= 0 or Auto_Skypiea == false
                            else
                                _tp(CFrame.new(980.09851074219, 121.33129882812, 1287.2093505859))
                            end
                        end
                ...
                elseif replicated.Remotes.CommF_:InvokeServer("Wenlocktoad", "1") == 1 then
                    for _, player in pairs(game.Players:GetChildren()) do
                        if player.Name ~= plr.Name and tostring(player.Data.Race.Value) == "Skypiea" then
                            ...
                            until player.Character.Humanoid.Health <= 0 or Auto_Skypiea == false
                        end
                    end
                end
            end
                    ...
                    elseif alchemist == 1 then
                        ...
                        elseif not GetBP("Flower 3") then
                            ...
                            if mob then
                                repeat
                                    wait()
                                    G.Kill(mob, Auto_Fish)
                                until GetBP("Flower 3") or not mob.Parent or mob.Humanoid.Health <= 0 or Auto_Fish == false
                            else
                                _tp(CFrame.new(980.09851074219, 121.33129882812, 1287.2093505859))
                            end
                        end
...
task.spawn(function()
    while task.wait(30) do
        if AutoNotifyFruit and CheckFruit then
            pcall(function()
                local inStock, price = CheckSpecificFruit(CheckFruit)
                if inStock then
                    Library:SetNotification({Title = "Fruit Stock Alert!", Description = "| Fruit Stock Alert!", Content = "", Time = 0.5, Delay = 10})
                end
            end)
        end
    end
...
task.spawn(function()
    while task.wait(5) do
        if AutoChipBeli and SelectChip then
            pcall(function()
                if not GetBP("Special Microchip") then
                    replicated.Remotes.CommF_:InvokeServer("RaidsNpc", "Select", SelectChip)
                end
            end)
        end
    end
...
task.spawn(function()
    while task.wait(5) do
        if AutoChipFruit and SelectChip then
            pcall(function()
                if not GetBP("Special Microchip") then
...
AutoDungeonFarm = false

task.spawn(function()
    while task.wait(0.3) do
        if not AutoDungeonFarm then
            continue
        end
...
AutoStartLawRaid = false

task.spawn(function()
    while task.wait(5) do
        if AutoStartLawRaid and World2 then
            pcall(function()
...
task.spawn(function()
    while task.wait(3) do
        if PortalUnLock and World3 then
            pcall(function()
...
task.spawn(function()
    while task.wait(1) do
        if AutoTime then
            Lighting.ClockTime = (SelectDN == "Day") and 12 or 0
        end
    end
end)
...
task.spawn(function()
    while task.wait(0.5) do
        if IceWalk and plr.Character and plr.Character:FindFirstChild("HumanoidRootPart") then
            pcall(function()
...
WalkSpeedEnabled = WalkSpeedEnabled or false
JumpEnabled = JumpEnabled or false
...
local function setupSpeedWatcher(hum)
    if hum then
        hum:GetPropertyChangedSignal("WalkSpeed"):Connect(function()
            if WalkSpeedEnabled and hum.WalkSpeed ~= (getgenv()).WalkSpeedValue then
                hum.WalkSpeed = (getgenv()).WalkSpeedValue
            end
        end)
    end
