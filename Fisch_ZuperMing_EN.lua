...
if getgenv().ZuperMingActive then
    ...
    -- Safety Unanchor on script reload
    ...
    if ReelModule then
        ...
        if success and Controller then
            ...
            Controller.StartReel = function(data)
                -- Skip if totem is currently active
...
local function RunAutoSpear()
    ...
    while Config.AutoSpear and getgenv().ZuperMingActive do
        ...
        -- Check SpearfishingZone (correct method from decompile)
        ...
        -- Fallback: check roaming fish if none in zone
...
task.spawn(function()
    while task.wait(0.5) do
        -- Double check: skip if totem is currently active
...
local function equipAndUseTotem(name)
    ...
    -- If already equipped on character
    for _, tool in pairs(char:GetChildren()) do
        if tool:IsA("Tool") and tool.Name == name then
            tool:Activate()
    ...
    print("-- [AUTO TOTEM] Totem not found: " .. name)
    Library:MakeNotify({Title = "Auto Totem", Content = "Totem not found!", Duration = 4})
    Config.TotemActive = false
end
...
local function RunAutoTotem()
    TotemThread = task.spawn(function()
        ...
        local lastUseTime = 0 -- Set to 0 so totem is used immediately when first enabled
        
        while Config.AutoTotem do
            task.wait(1) -- Loop checks every 1 second for fast response when time changes

            -- Check if there's an active event (Busy flag)
            if Config.TotemBusy then
                if tick() - TotemBusyTimer >= TOTEM_BUSY_DURATION then
                    Config.TotemBusy = false
                    print("-- [AUTO TOTEM] Clear, continuing activation...")
                else
                    continue
                end
            end
            ...
            -- 2. Check if time just changed
            local isTimeChanged = (currentTime ~= lastTimeOfDay)

            -- 3. Execute IF time changed OR delay reached
            if isTimeChanged or (tick() - lastUseTime >= Config.TotemDelay) then
                ...
                if totemToUse ~= "None" and totemToUse ~= "" then
                    if isTimeChanged then
                        print("-- [AUTO TOTEM] Time changed to " .. currentTime .. "! Using: " .. totemToUse)
                    else
                        print("-- [AUTO TOTEM] Time: " .. currentTime .. " | Normal Activating: " .. totemToUse)
                    end
                    equipAndUseTotem(totemToUse)
                end

                -- 4. Reset time tracker and delay timer
                lastUseTime = tick()
...
-- Totem options ("None" added at front for manual disable)
local TotemOptions = {
...
-- ============================================================
-- Declared at top of script so it can be accessed On/Off
local RadarLoop = nil
...
-- Relics that use SECONDARY enchant slot (Cosmic & Twisted)
...
task.spawn(function()
    while true do
        ...
        if Config.AutoEnchant and getgenv().ZuperMingActive then
            ...
            -- Cosmic & Twisted use secondary slot, others use primary
            ...
            if currentEnchant and string.find(string.lower(currentEnchant), string.lower(Config.TargetEnchant)) then
                ...
                Library:MakeNotify({
                    ...
                    Content = "Successfully obtained: " .. currentEnchant .. " on " .. currentRod.Name, 
                    Duration = 8, 
                    Icon = "check"
                })
                print("✅ [AUTO ENCHANT] Target achieved:", currentEnchant)
            else
                -- 1. Teleport to Altar if too far
                ...
                if hasRelic then
                    ...
                    if altarFired then
                        task.wait(0.5) -- Wait for UI Dialog to appear
                        
                        -- 4. Auto Confirm using getconnections
                        pcall(function()
                            local GuiService = game:GetService("GuiService")
                            local VirtualInputManager = game:GetService("VirtualInputManager")
                            ...
                            if not confirmBtn then
                                warn("[Enchant] confirm button not found!")
                                return
                            end
                    ...
                    Library:MakeNotify({
                        ...
                        Content = Config.SelectedRelic .. " is empty or not found in backpack!", 
                        Duration = 5
                    })
                end
            end
...
-- Sort mutation names from longest for more accurate word detection
local DetectionList = {}
...
local function ParseAppraiseResult(text)
    local cleanText = text:gsub("<[^>]*>", "") -- Remove HTML/RichText tags
    local hasShiny = cleanText:find("Shiny") ~= nil
    local hasSparkling = cleanText:find("Sparkling") ~= nil
    local hasBig = cleanText:find("Big") ~= nil -- Detect Big
...
local function RunAutoAppraise()
    ...
    task.spawn(function()
        -- 1. Save original position & teleport to Appraiser
        local originalCFrame = hrp.CFrame
        hrp.CFrame = CFrame.new(448.84, 150.54, 207.15) * CFrame.Angles(0, math.rad(-86.58), 0)
        task.wait(0.5) -- Wait for character to fully arrive so prompt appears

        -- 2. Find and trigger Proximity Prompt
        ...
        if prompt then
            ...
            print("Appraiser Prompt not found!")
            Library:MakeNotify({Title = "Error", Content = "Failed to interact with NPC!", Duration = 3})
            Config.AutoAppraise = false
            return
        end
        ...
        while Config.AutoAppraise do
            ...
            -- Check fish text in character Subvalues UI
            ...
            if subvalues and subvalues:IsA("TextLabel") then
                ...
                print("🔍 Checking fish:", result.fullText)
...
local function GetEventText()
    ...
    -- Use FindFirstChild to be safe and avoid script freeze
    ...
    for _, eventName in ipairs(eventList) do
        ...
        -- utf8.char(9989) is ✅, utf8.char(10060) is ❌
...
local snapMutationList = {
    ...
    --  STATE
    -- =============================================
    local FishByName = {}   -- dynamic from inventory: name → { uid, ... }

    local SelName     = {}
    local SelRarity   = {}  -- original values from rarityList
    local SelMutation = {}  -- original values from mutationList

    -- =============================================
    --  BUILD fish names from inventory (dynamic)
    -- =============================================
    ...
    local function CollectUIDs()
        ...
        -- Collect UIDs from rarity & mutation filter (direct inventory scan)
        if #SelRarity > 0 or #SelMutation > 0 then
            ...
            for uid, itemData in pairs(inventory) do
                ...
                if not itemData.sub then continue end
                if typeof(itemData.sub.Weight) ~= "number" then continue end
                if seen[uid] then continue end  -- already added from name filter
    ...
    local function RefreshDropdown()
        ...
        Library:MakeNotify({
            Title    = "Refresh",
            Content  = total .. " fish found in backpack!",
            Duration = 3
        })
    end
    ...
    local function RunAutoStorage()
        ...
        if not lastCFrame then
            Library:MakeNotify({Title = "Error", Content = "Character not found!", Duration = 3})
            return
        end
        ...
        -- 2. Teleport to storage (once)
        TeleportTo(STORAGE_CFRAME)

        -- 3. Wait until near storage (max 3 seconds)
        local waited = 0
        ...
        -- 6. Teleport back to original position
        ...
        if count > 0 then
            Library:MakeNotify({
                Title    = "Auto Storage",
                Content  = count .. " fish stored! Returning to original position.",
                Duration = 3
            })
        else
            Library:MakeNotify({
                Title    = "Auto Storage",
                Content  = "No fish to store.",
                Duration = 3
            })
        end
    ...
    local function RunStarCrater()
        ...
        Library:MakeNotify({
            Title   = "Star Crater",
            Content = #craters .. " craters found, starting claim...",
            Duration = 2
        })
        ...
        for _, crater in pairs(craters) do
            ...
            for _, prompt in pairs(prompts) do
                ...
                -- Teleport to prompt parent position
                local part = prompt.Parent
                local char = LocalPlayer.Character
                if char then
                    local hrp = char:FindFirstChild("HumanoidRootPart")
                    if hrp then
                        -- Use .Position not .WorldPosition
                        hrp.CFrame = CFrame.new(part.Position + Vector3.new(0, 3, 0))
                        task.wait(0.5)
                    end
                end

                -- Fire proximity with pcall
        ...
        Library:MakeNotify({
            Title    = "Star Crater",
            Content  = totalClaim .. " items claimed successfully!",
            Duration = 4
        })
    end
...
local Window = Library:Window({Footer = "Fisch - 1.4"})
...
local function LoadMainTab()
    ...
    FarmSection:AddToggle({
        ...
        Content = "Auto equip rod from backpack",
        Default = false,
    ...
    FarmSection:AddToggle({ 
        Title = "Auto Cast", 
        Content = "Auto cast when bobber is empty",
        Default = false, 
    ...
    FarmSection:AddToggle({ 
        Title = "Auto Shake", 
        Content = "Center button + auto FireServer shake",
        Default = false, 
    ...
    SnapSection:AddToggle({
        ...
        Content = "Auto cancel if fish does not match filter",
        Default = false,
        Callback = function(v)
            Config.SnapReel = v
            if v then
    ...
    SpearSection:AddToggle({
        ...
        Callback = function(Value)
            ...
            if Value then
                ...
                Library:MakeNotify({
                    Title = "Auto Spear",
                    Content = "Started at: " .. Config.AutoSpearLocation,
                    Duration = 3
                })
            else
                Library:MakeNotify({
...
local function LoadAutoTab()
    ...
    AutoAppraise:AddToggle({
        ...
        Content = "Auto Teleport & Gacha",
        Default = false,
        Callback = function(v)
            Config.AutoAppraise = v
            if v then
                -- Prevent script from running if nothing is selected
                if Config.ShinyType == "None" and Config.TargetMutation == "None" and Config.TargetSize == "None" then
                    Library:MakeNotify({Title = "Error", Content = "Please select a Mutation, Tier, or Size target first!", Duration = 3})
                    Config.AutoAppraise = false
                    return
                end
                RunAutoAppraise()
    ...
    TotemSection:AddToggle({
        ...
        Content = "Automatically switch totem based on time",
        Default = false,
        Callback = function(v)
            Config.AutoTotem = v
            if v then
    ...
    -- Combine all enchants into one list so UI doesn't need dynamic updates (Anti-Bug)
    -- Enchant dropdown reference for dynamic updates
    local EnchantDropdownRef = nil

    -- Function to update enchant dropdown based on selected relic
    ...
    EnchantSection:AddDropdown({
        ...
        Callback = function(Value)
            ...
            -- Auto update enchant list when relic is changed
            UpdateEnchantDropdown(Value)
        end
    })

    -- Dropdown 2: Target Enchant (initial list from Enchant Relic)
    ...
    EnchantSection:AddToggle({
        ...
        Content = "Auto Equip Relic, Teleport, & Gacha Altar",
        Default = false,
        Callback = function(Value)
            ...
            if Value then
                Library:MakeNotify({
                    Title = "Auto Enchant",
                    Content = "Started! Searching: " .. (Config.TargetEnchant or "?") .. " [" .. (Config.SelectedRelic or "?") .. "]",
                    Duration = 3
                })
            end
        end
    ...
    -- =============================================
    --  REFRESH BUTTON (only refresh fish names)
    -- =============================================
    FavSection:AddButton({
        Title    = "Refresh Fish Names",
        Content  = "Update fish name list from inventory",
        Callback = function()
            BuildFishNames()
            SelName = {}
    ...
    FavSection:AddToggle({
        ...
        Content  = "Favourite all fish from selected filter",
        Default  = false,
        Callback = function(v)
            ...
            if not hasFilter then
                Library:MakeNotify({
                    Title   = "Warning",
                    Content = "Please select at least 1 filter first!",
                    Duration = 3
                })
                return
            end
            ...
            Library:MakeNotify({
                Title    = "Auto Favourite",
                Content  = count .. " fish favourited successfully!",
                Duration = 4,
                Icon     = "star"
            })
        end
    ...
    StorageSection:AddButton({
        Title    = "Refresh Fish",
        Content  = "Update fish list from backpack",
        Callback = function()
            RefreshDropdown()
        end
    })
    ...
    StorageSection:AddToggle({
        Title    = "Auto Storage",
        Content  = "Save fish to storage then return to original position",
        Default  = false,
        Callback = function(v)
            if v then
                if #StorageSelName == 0 then
                    Library:MakeNotify({
                        Title   = "Warning",
                        Content = "Please select at least 1 fish first!",
                        Duration = 3
                    })
                    return
                end
                ...
                Library:MakeNotify({
                    Title   = "Auto Storage",
                    Content = "Starting storage process...",
                    Duration = 2
                })
            else
                StorageActive = false
    ...
    AutoSection:AddButton({
        ...
        Content  = "Claim all items in Starfall then return to original position",
        Callback = function()
    ...
    local function AutoCollectAllChests()
        ...
        if #allChests == 0 then
            Library:MakeNotify({Title = "Chest Farmer", Content = "No chests available right now.", Duration = 3})
            return
        end
    
        Library:MakeNotify({Title = "Chest Farmer", Content = "Starting collection of " .. #allChests .. " chests...", Duration = 5})
            ...
            if chest:IsA("Model") or chest:IsA("BasePart") then
                ...
                if rootPart then
                    ...
                    if targetPrompt then
                        ...
                        print("✅ Successfully collected: " .. chest.Name)
                    end
        ...
        Library:MakeNotify({Title = "Chest Farmer", Content = "Finished collecting all chests!", Duration = 5, Icon = "check"})
    end
...
local function LoadShopTab()
    ...
    BaitSection:AddButton({
        ...
        Callback = function()
            if Config.SelectedShopItem then
                ...
                -- Loop to buy according to input amount
                for i = 1, amount do
                    PurchaseEvent:FireServer(Config.SelectedShopItem, "Fish", nil, 1)
                    -- Very small delay to avoid server rate limit
    ...
    RemoteShopSection:AddButton({
        ...
        Content = "Purchase selected item remotely",
        Callback = function()
    ...
    ShopUISection:AddButton({
        ...
        Content = "Open Daily Shop menu remotely",
        Callback = function()
    ...
    ShopUISection:AddButton({
        ...
        Content = "Open Black Market menu remotely",
        Callback = function()
            ...
            if blackMarketGui then
                ...
                Library:MakeNotify({Title = "Error", Content = "Black Market UI not found!", Duration = 3})
            end
        end
    })
...
local function LoadTeleportTab()
    ...
    TeleportSection:AddButton({
        ...
        Callback = function()
            if Config.SelectedLoc then
                local char = LocalPlayer.Character
                if char and char:FindFirstChild("HumanoidRootPart") then
                    -- Change: Use Config.SelectedLoc directly since it's already a CFrame
    ...
    WhirlpoolSection:AddButton({
        ...
        Content = "Find and TP to nearest Whirlpool",
        Callback = function()
                ...
                if targetWhirlpool then
                    ...
                    if hrp then
                        ...
                        if targetWhirlpool:IsA("Model") then
                            -- If Model, use GetModelCFrame() or PrimaryPart
                        ...
                        elseif targetWhirlpool:IsA("BasePart") then
                            -- If regular Part, get .CFrame directly
                            targetCFrame = targetWhirlpool.CFrame
                        end
    
                        if targetCFrame then
                            -- Offset +5 studs upward to avoid clipping
                            ...
                            print("Cannot get CFrame from Safe Whirlpool!")
                        end
                    end
                else
                    Library:MakeNotify({
                        Title = "Whirlpool Not Found",
                        Content = "Whirlpool is not active on this server.",
                        Duration = 3
                    })
                end
            else
                print("workspace.active folder not found!")
            end
        end
    })
    ...
    local playerList = {}
    local tpLoopThread -- Variable to store loop thread
    ...
    local function GetTargetCFrame(targetName)
        ...
        -- Method 1: Try to get Model position (best for StreamingEnabled)
        local pivot = char:GetPivot()
        if pivot and pivot.Position.Y > -500 then -- Check position validity
            return pivot
        end

        -- Method 2: Find RootPart (Fallback)
        local hrp = char:FindFirstChild("HumanoidRootPart")
        if hrp then return hrp.CFrame end

        -- Method 3: Find Torso (R6/R15 Fallback)
...
local function LoadMiscTab()
    ...
    PlayerUtilities:AddToggle({
        Title = "Walk On Water",
        Content = "Allow player to walk on water",
        Default = false,
        Callback = function(Value)
            wowEnabled = Value
            if Value then
    ...
    GeneralFeatures:AddToggle({
        Title = "Anti AFK",
        Content = "Prevent 20-minute kick",
        Default = true,
        Callback = function(Value)
            if Value then
                StartAntiAFKLogic()
